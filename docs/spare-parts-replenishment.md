...
| خطوة | من يفعل | تأثير المخزون | ملاحظة واجهة |
|------|---------|----------------|---------------|
| طلب | المركز | لا | طباعة متاحة من المقدّم |
| اعتماد | مسؤول الاعتماد | لا | |
| تجهيز | المخزن المركزي | لا | يمكن تقليل الكمية أو استبعاد بند (`preparedQty = 0`)؛ الحالة: **جاهز (مجهّز)** |
| موافقة المسؤول / تأكيد الخروج | المسؤول | لا | الحالة: **خرج — بانتظار استلام المركز** |
| تأكيد الاستلام | المركز | **نعم** — TRANSFER من المركزي → المركز | البنود المستبعدة لا تُرحَّل |
...
- في إنشاء طلب التموين / سحب المركزي تظهر فقط المواد المفعّلة صراحةً كقطع غيار (`availableForSpareParts = true`) — مواد التصنيع العادية (مثل أكواد MAT) لا تظهر في القائمة.
