...
export function downloadProductComponentsTemplate() {
  ...
  const rows: (string | number)[][] = [
    ...
    ['SK-999N', 'MAT-001', 'موتور نحاس', 1, '', 'مكون', 18, '20-01-0', '20-01-0', 100],
    ['SK-999N', 'CTN-999', 'كرتون خارجي 999', '', 6, 'تعبئة وتغليف', '', '', '', 200],
    ['PRD-SP-01', 'MAT-498', 'بودي', '', '', '', '', '', '', ''],
    ['PRD-002', 'MAT-003', 'جلدة مانعة للتسرب', 2, '', 'مكون', 1.2, '', '', ''],
  ];
  ...
  const guideAoa: (string | number)[][] = [
    ...
    ['كمية الاستخدام', 'رقم > 0 للمنتج التصنيعي؛ فاضي/صفر = قطعة غيار بدون استهلاك تصنيع. للكرتونة الخارجية: اتركها فاضية واكتب «عدد الوحدات في الكرتونة» — يُحسب استهلاك الكرتونة 1÷العدد لكل قطعة'],
    ['عدد الوحدات في الكرتونة', 'اختياري — يُكتب على بطاقة المنتج (قطع لكل كرتونة). للكرتون: هذا هو عمود «العدد»'],
    ...
    ['الكمية المستخدمة', 'لا — فاضي/صفر = كتالوج قطع صيانة؛ فاضي مع عدد/كرتونة = استهلاك 1÷العدد'],
    ['عدد الوحدات في الكرتونة', 'لا — يُحدّث بطاقة المنتج (كم قطعة في الكرتونة)'],
    ['نوع المادة', 'لا — مكون | نصف مصنع | مستهلكات | تعبئة وتغليف (للكرتون: تعبئة وتغليف)'],
    ['تكلفة الوحدة', 'لا'],
...
export const MATERIALS_TEMPLATE_SHEET_NAME = 'المواد التصنيعية';

/** Excel rows for the manufacturing materials import template (header + samples). */
export function getMaterialsTemplateSheetRows(): (string | number)[][] {
  return [
    [
      'مصدر المادة',
      'كود المادة',
    ...
    ['شراء خارجي', 'MAT-001', 'موتور نحاس', 'كهرباء', 'مكون', 'قطعة', 'قطعة', 1, 18, 2, 10, '', 'نعم', 'نشط'],
    ['شراء خارجي', 'MAT-002', 'خام بلاستيك', 'بلاستيك', 'مكون', 'كجم', 'كجم', 1, 60, 0, 20, '', 'نعم', 'نشط'],
...
export function downloadMaterialsTemplate() {
  const wb = XLSX.utils.book_new();
  const rows = getMaterialsTemplateSheetRows();
  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws['!cols'] = [
    { wch: 18 },
    { wch: 14 },
  ...
  if (!ws['!views']) ws['!views'] = [];
  (ws['!views'] as any[]).push({ rightToLeft: true });
  XLSX.utils.book_append_sheet(wb, ws, MATERIALS_TEMPLATE_SHEET_NAME);
  ...
  const guideAoa: (string | number)[][] = [
    ...
    ['النوع', 'لا — مكون | نصف مصنع | مستهلكات | تعبئة وتغليف'],
    ...
    ['تظهر في قطع الغيار', 'لا — نعم | لا (الافتراضي للجديد: لا)'],
    ['الحالة', 'لا — نشط | موقوف'],
