...
export function downloadProductComponentsTemplate() {
  ...
  const guideAoa: (string | number)[][] = [
    ...
    ['كمية الاستخدام', 'رقم > 0 للمنتج التصنيعي؛ فاضي/صفر = ربط بدون استهلاك تصنيع. للكرتونة: اتركها فاضية — عمود العدد يروح على بطاقة المنتج ولا يُقسم الرصيد'],
    ['عدد الوحدات في الكرتونة', 'اختياري — العدد الداخلي للقطعة (كم قطعة داخل الكرتونة). يُكتب على بطاقة المنتج'],
    ...
    ['الكمية المستخدمة', 'لا — فاضي/صفر = ربط بدون استهلاك؛ للكرتون اتركها فاضية'],
