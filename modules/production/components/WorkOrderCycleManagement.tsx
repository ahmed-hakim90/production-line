...
const actionLabels: Record<string, string> = { prepare: 'تجهيز الأمر', editDraft: 'تعديل المسودة والخطة', approve: 'اعتماد الأمر', reassignSupervisor: 'إعادة إسناد المشرف', assignInspectors: 'تكليف مراقبي الجودة', assignWorkers: 'ربط العمالة', start: 'بدء الساعة', submit: 'تسجيل الإنتاج', pause: 'تسجيل توقف', resume: 'استئناف التشغيل', defineQualityReport: 'تعريف نموذج الجودة', submitQualityReport: 'حفظ مشاركة الجودة', claimQualityInspection: 'حجز فحص الحاوية', releaseQualityInspection: 'فك حجز الفحص', approveQualityReport: 'اعتماد نتيجة الفحص', returnQualityReport: 'إعادة النتيجة للفحص', correctQualityReport: 'تصحيح نتيجة الفحص', lockQuality: 'قفل تشغيل الأمر', unlockQuality: 'فك قفل التشغيل', decideRejectedDisposition: 'قرار المرفوض (إعادة تشغيل/هالك)', submitRework: 'تسجيل إنتاج إعادة التشغيل', claimReworkInspection: 'حجز فحص إعادة التشغيل', submitReworkQualityReport: 'حفظ نتيجة فحص إعادة التشغيل', approveReworkQualityReport: 'اعتماد نتيجة إعادة التشغيل', receivePackaging: 'استلام للتغليف', packageContainer: 'تعبئة الحاوية', deliverToWarehouse: 'تسليم للمخزن', proposePlanRevision: 'اقتراح توزيع جديد للساعات', applyPlanRevision: 'اعتماد توزيع الساعات الجديد', closeProduction: 'إقفال الإنتاج' };
export function CycleAudit({ order }: { order: CycleOrder }) {
  return <section className="space-y-3 rounded-lg border border-border bg-card p-4"><h2 className="font-semibold">سجل إجراءات أمر الشغل</h2>{order.auditTruncated && <p role="status">يعرض آخر ١٠٠ إجراء فقط؛ السجل الأقدم محفوظ.</p>}{order.audit?.length ? <ol className="max-h-96 space-y-3 overflow-auto">{order.audit.map(event => <li key={event.id} className="space-y-1 border-b border-border pb-3"><p>{actionLabels[event.action] || event.action} — {event.actorName}</p><p className="text-sm text-muted-foreground"><time dateTime={event.createdAt}>{new Date(event.createdAt).toLocaleString('ar-EG')}</time> • إصدار {event.revision}</p>{event.reason && <p className="text-sm">السبب: {event.reason}</p>}{event.supervisorUid && <p className="text-sm">المشرف السابق: {event.previousSupervisorUid} ← الجديد: {event.supervisorUid}</p>}</li>)}</ol> : <p>لا توجد إجراءات مسجلة.</p>}</section>;
}
