...
export const RepairSpareIssuesSettingsSection: React.FC<Props> = ({
  isAdmin,
  localPlanSettings,
  ...
  const mode = localPlanSettings.repairSpareIssueApprovalMode === 'required'
    ? 'required'
    : 'direct';
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-base font-bold text-[var(--color-text)]">صرف قطع غيار الصيانة</h3>
        <p className="text-xs text-[var(--color-text-muted)] mt-1">
          إعداد عام للشركة يحدد إن كان سند صرف قطع الغيار من مخزن مركز الصيانة ينفّذ مباشرة أو يحتاج موافقة قبل خصم الرصيد.
        </p>
      </div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setLocalPlanSettings((prev) => ({
                ...prev,
                repairSpareIssueApprovalMode: next,
              }));
            }}
          >
