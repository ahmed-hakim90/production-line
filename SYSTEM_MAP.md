...
4. **Repair** (parallel): receive (per-product `inWarranty`) → **desk assign or QR claim** (`/repair/jobs/:jobId/claim` → workspace; unassigned `received` advances to `diagnosing` / جاري الفحص; QR claim is tenant-scoped for any `repair.jobs.technician`, not limited to branch `technicianIds`) → technician saves diagnosis → `diagnosed` / تم الفحص → part/service → `estimate_ready` → customer approval → repair/parts → ready → deliver. Desk (reception/edit) may **reassign / فك الإسناد** when the tech is unavailable; unassign on diagnosing without diagnosis rolls back to `received`. Closed/terminal QR reopen is view-only when already assigned to that tech. Status roles configured in repair settings. Manufacturer warranty: all lines → full `WAR-…` close without collect; mixed → bill non-warranty only + `warrantyAllowances` on deliver  
