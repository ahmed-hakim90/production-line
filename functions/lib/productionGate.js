...
export const syncProductionGateEmployeeCache = onCall({ region: 'us-central1', memory: '256MiB' }, async (request) => {
    const actor = await loadActor(request);
    requirePermission(actor, 'production.gate.register');
...
export const registerProductionGateAction = onCall({ region: 'us-central1', memory: '256MiB' }, async (request) => {
    ...
    if (!/^[A-Za-z0-9_-]{8,80}$/.test(eventId))
        throw new HttpsError('invalid-argument', 'معرّف الحركة غير صالح.');
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (state?.openSessionId) {
            ...
            if (openSnap.exists && openSnap.data()?.status === 'open') {
                if (data.expectedAction && data.expectedAction !== 'entry')
                    throw new HttpsError('aborted', 'حالة العامل تغيرت؛ راجع الحركة قبل المزامنة.');
        ...
        if (data.expectedAction && data.expectedAction !== 'exit')
            throw new HttpsError('aborted', 'حالة العامل تغيرت؛ راجع الحركة قبل المزامنة.');
        tx.set(sessionRef, {
