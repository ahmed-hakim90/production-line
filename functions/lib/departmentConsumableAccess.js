...
const DEPARTMENT_CONSUMABLE_ALIASES = {
...
export function resolveConsumableActorRoleKey(role) {
    ...
    if (name === MATERIALS_WAREHOUSE_ROLE_NAME)
        return MATERIALS_WAREHOUSE_ROLE_KEY;
    return null;
}
export function actorHasDepartmentConsumableAccess(actor, keys) {
    if (actor.isSuperAdmin)
        return true;
    if (keys.some((key) => actor.permissions[key] === true))
        return true;
    for (const key of keys) {
        const aliases = DEPARTMENT_CONSUMABLE_ALIASES[key] || [];
        if (aliases.some((alias) => actor.permissions[alias] === true))
            return true;
    }
