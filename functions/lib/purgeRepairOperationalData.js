...
async function actorMayPurge(uid) {
...
async function deleteSubcollection(parent, subcollection) {
    let deleted = 0;
    while (true) {
...
async function deleteRepairJobsWithEvents(tenantId) {
    let jobs = 0;
    let events = 0;
    while (true) {
...
async function deleteByTenant(collectionName, tenantId) {
    return deleteQueryBatch(db.collection(collectionName).where('tenantId', '==', tenantId));
}
async function deleteRepairStockTransactions(tenantId) {
    const moduleSet = new Set(REPAIR_STOCK_SOURCE_MODULES);
    let deleted = 0;
    let last;
    ...
    while (true) {
        ...
        const matches = snap.docs.filter((row) => moduleSet.has(String(row.data()?.sourceModule || '')));
        if (matches.length > 0) {
            const batch = db.batch();
...
export const purgeRepairOperationalData = onCall({
    ...
    if (confirmPhrase !== expected) {
        throw new HttpsError('invalid-argument', `للتأكيد اكتب بالضبط: ${expected}`);
    }
    const deletedCounts = {};
    ...
    deletedCounts.repair_counters = await deleteQueryBatch(db.collection('repair_counters').where('tenantId', '==', tenantId));
