...
const attendanceId = (reportId, subject) => (`${reportId}_${encodeURIComponent(subject)}`);
const buildAttendanceRows = (reportId, report) => {
    if (report.lifecycleStatus === 'open')
        return [];
    const common = {
...
const materializeAttendance = async (reportId, report) => {
    ...
    if (!existing.empty || rows.length > 0)
        await batch.commit();
};
const countsTowardProgress = (report) => {
    const type = clean(report.reportType) || 'finished_product';
    return type !== 'packaging' && type !== 'component_waste';
};
const reconcileWorkOrder = async (report) => {
    const workOrderId = clean(report.workOrderId);
    if (!workOrderId || !countsTowardProgress(report))
        return;
...
const reconcilePlan = async (report) => {
    const planId = clean(report.productionPlanId);
    if (!planId || !countsTowardProgress(report))
        return;
...
const shouldApplyInventory = async (tenantId) => {
...
const calculateAndPostLegacyCost = async (reportId, report) => {
    ...
    const centerById = new Map(centersSnap.docs.map((snapshot) => [snapshot.id, snapshot.data()]));
    const allocationByCenter = new Map(allocationsSnap.docs
        .map((snapshot) => snapshot.data())
        .filter((row) => clean(row.month) === month)
        .map((row) => [clean(row.costCenterId), row]));
    let lineIndirect = 0;
    const indirectByCenter = {};
    for (const valueSnapshot of valuesSnap.docs) {
        const value = valueSnapshot.data();
        if (clean(value.month) !== month)
            continue;
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!reportSnap.exists)
            throw new Error('التقرير غير موجود أثناء ترحيل التكلفة.');
        const current = reportSnap.data();
        if (clean(current.tenantId) !== tenantId)
            throw new Error('التقرير خارج الشركة أثناء ترحيل التكلفة.');
...
const claim = async (reportId) => {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        if (report.processingState !== 'pending')
            return null;
        transaction.set(ref, {
...
export async function processProductionReportBackground(reportId) {
    ...
    try {
        ...
        if (await shouldApplyInventory(clean(report.tenantId))) {
            ...
            if (!creatorUid)
                throw new Error('لا يوجد منشئ مسجل لتنفيذ ترحيل المخزون.');
...
export const shouldProcessProductionReportUpdate = (before, after) => (clean(before.data()?.processingState) !== 'pending'
    && clean(after.data()?.processingState) === 'pending');
