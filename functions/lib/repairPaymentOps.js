...
const sumPartsActualCost = (rows) => roundMoney((Array.isArray(rows) ? rows : []).reduce((sum, raw) => {
    const row = raw;
    const totalSnapshot = Number(row.totalCostSnapshot);
    if (Number.isFinite(totalSnapshot) && totalSnapshot >= 0)
        return sum + totalSnapshot;
...
const loadScopedJob = async (actor, jobId) => {
...
const discountTotals = (grossInput, typeInput, valueInput) => {
    const grossAmount = roundMoney(grossInput);
    const type = typeInput === 'amount' || typeInput === 'percent' ? typeInput : 'none';
...
const computeBreakdown = async (tenantId, job) => {
    ...
    if (unlinkedUsage) {
        throw new HttpsError('failed-precondition', `اربط قطعة «${String(unlinkedUsage.partName || 'غير معروفة')}» بجدول المواد قبل التسعير.`);
    }
...
const paymentStatus = (net, paid) => {
    if (net <= 0 || paid >= net - 0.001)
        return 'paid';
    return paid > 0 ? 'partial' : 'unpaid';
};
const accountSeeds = (tenantId, configured) => ({
...
const requireAccountingMap = (value) => {
...
const assertAccountingMapReady = async (tenantId, value) => {
    const map = requireAccountingMap(value);
    const keys = ['cash', 'card', 'bankTransfer', 'customerDeposits', 'receivables', 'serviceRevenue', 'partsRevenue', 'discounts', 'warrantyAllowances', 'partsInventory', 'partsCogs'];
    const snapshots = await db.getAll(...keys.map((key) => db.collection('accounting_accounts').doc(`${tenantId}__${String(map[key] || '').trim()}`)));
    ...
    if (invalidKey) {
        throw new HttpsError('failed-precondition', `حسابات فرع الصيانة غير مكتملة (حقل: ${invalidKey}). من الحسابات ← إعدادات الحسابات: اختر مركز التكلفة ثم «حفظ الربط بالحسابات الافتراضية».`);
    }
    return map;
};
const ensureWarrantyAllowanceMapping = async (actor, branchRef, configured) => {
 ...
 */
const loadLiveBranchAccounting = async (actor, branchId) => {
    const branchRef = db.collection('repair_branches').doc(branchId);
    const branchSnap = await branchRef.get();
...
const sanitizeJobTechnicalData = (job) => {
...
async function prepareAuthorization(actor, data) {
    ...
    if (!allowedStatuses.has(status)) {
        throw new HttpsError('failed-precondition', 'يُجهز إذن الدفع بعد اكتمال التقدير الفني، ولا يُحصّل إلا بعد الجاهزية الفنية.');
    }
    ...
    requirePermission(actor, estimatePhase
        ? ['repair.jobs.reception', 'repair.payments.collect', 'repair.discounts.request']
        : ['repair.payments.collect', 'repair.discounts.request'], 'ليس لديك صلاحية تجهيز إذن الدفع.');
    const costCenterId = String(scoped.branch.costCenterId || '').trim();
    if (!costCenterId)
        throw new HttpsError('failed-precondition', 'اربط الفرع بمركز تكلفة قبل تجهيز إذن الدفع.');
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (authSnap.exists)
            return { authorizationId, authorizationNo: String(authSnap.data()?.authorizationNo || '') };
    ...
    return { ok: true, ...result, ...totals, ...breakdown, settlementType: warrantyJob ? WARRANTY_SETTLEMENT : 'standard' };
}
async function requestCustomerApproval(actor, data) {
    ...
    if (!authorizationId)
        throw new HttpsError('failed-precondition', 'جهز إذن الدفع قبل إرسال موافقة العميل.');
    const authRef = db.collection('repair_payment_authorizations').doc(authorizationId);
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (isWarrantySettlement(auth) || isWarrantySettlement(currentFinSnap.data())) {
            throw new HttpsError('failed-precondition', 'طلب ضمان المصنّع لا يُرسل لموافقة تسعير العميل لأنه يحصل على إعفاء ضمان كامل تلقائيًا.');
        }
    ...
    return { ok: true, token, authorizationId, expiresAt };
}
async function resolveApproval(actor, data) {
    ...
    if (!approvalId || !decision)
        throw new HttpsError('invalid-argument', 'بيانات قرار الاعتماد غير مكتملة.');
    const approvalRef = db.collection('repair_financial_approvals').doc(approvalId);
    const initial = await approvalRef.get();
    if (!initial.exists)
        throw new HttpsError('not-found', 'طلب الاعتماد غير موجود.');
    const approval = initial.data();
    ...
    if (String(approval.tenantId || '') !== actor.tenantId)
        throw new HttpsError('permission-denied', 'الاعتماد خارج شركتك.');
    if (String(approval.requestedBy || '') === actor.uid)
        throw new HttpsError('failed-precondition', 'لا يمكن لمقدم الطلب اعتماد طلبه.');
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!approvalSnap.exists || !authSnap.exists || !finSnap.exists)
            throw new HttpsError('failed-precondition', 'مستندات الاعتماد غير مكتملة.');
        const current = approvalSnap.data();
...
async function requestCredit(actor, data) {
    ...
    const auth = authSnap.data();
    if (String(auth.tenantId || '') !== actor.tenantId)
    ...
    if (balance <= 0)
        throw new HttpsError('failed-precondition', 'لا يوجد رصيد متبقٍ يحتاج اعتمادًا.');
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (current.exists && String(current.data()?.status || '') === 'pending')
            return;
        tx.set(approvalRef, {
            tenantId: actor.tenantId,
    ...
    return { ok: true, approvalId, requestedAmount: balance };
}
async function collectPayment(actor, data) {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (!authSnap.exists || !finSnap.exists || !sessionSnap.exists)
            throw new HttpsError('failed-precondition', 'بيانات التحصيل غير مكتملة.');
        ...
        const auth = authSnap.data();
        if (isWarrantySettlement(auth) || isWarrantySettlement(finSnap.data())) {
            throw new HttpsError('failed-precondition', 'إذن ضمان المصنّع لا يُحصَّل. استخدم التسليم مباشرة بعد تجهيز إقفال الضمان.');
        }
        if (!['approved', 'partial'].includes(String(auth.status || '')))
            throw new HttpsError('failed-precondition', 'إذن الدفع غير معتمد للتحصيل.');
        if (String(auth.discountApprovalStatus || 'approved') !== 'approved')
            throw new HttpsError('failed-precondition', 'الخصم لم يعتمد بعد.');
        ...
        if (amount > balance + 0.001)
            throw new HttpsError('invalid-argument', 'مبلغ الدفعة أكبر من الرصيد المتبقي.');
...
async function reversePayment(actor, data) {
    ...
    if (!paymentId || reason.length < 3)
        throw new HttpsError('invalid-argument', 'سبب العكس مطلوب.');
    const paymentRef = db.collection('repair_payments').doc(paymentId);
    const initial = await paymentRef.get();
    if (!initial.exists)
        throw new HttpsError('not-found', 'الدفعة غير موجودة.');
    const payment = initial.data();
    if (String(payment.tenantId || '') !== actor.tenantId)
        throw new HttpsError('permission-denied', 'الدفعة خارج شركتك.');
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!paymentSnap.exists || !authSnap.exists || !finSnap.exists)
...
async function deliver(actor, data) {
    ...
    const initialFin = initialFinSnap.data();
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (!jobSnap.exists || !finSnap.exists || !authSnap.exists)
            throw new HttpsError('failed-precondition', 'بيانات التسليم غير مكتملة.');
        const job = jobSnap.data();
        ...
        if (String(job.status || '') !== 'ready')
            throw new HttpsError('failed-precondition', 'التسليم مسموح بعد حالة جاهز للتسليم فقط.');
        const hasPendingParts = (Array.isArray(job.partsUsed) ? job.partsUsed : []).some((raw) => ['pending_supply', 'ready_to_issue'].includes(String(raw.fulfillmentStatus || '')));
        if (hasPendingParts)
            throw new HttpsError('failed-precondition', 'لا يمكن التسليم قبل صرف كل القطع المعلقة.');
...
export const mutateRepairPaymentHandler = async (request) => {
    const actor = await loadActor(requireAuth(request));
    const data = (request.data || {});
    const operation = String(data.operation || '');
    if (operation === 'prepare')
        return prepareAuthorization(actor, data);
    if (operation === 'resolve_approval')
        return resolveApproval(actor, data);
    if (operation === 'request_credit')
    ...
    if (operation === 'deliver')
        return deliver(actor, data);
    if (operation === 'request_customer_approval')
        return requestCustomerApproval(actor, data);
