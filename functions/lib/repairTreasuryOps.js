...
const requireAdminTreasury = (actor) => {
    if (actor.isSuperAdmin
        || actor.permissions['repair.branches.manage'] === true
        || actor.permissions['repair.callCenter.viewAll'] === true) {
        return;
...
const resolveMainBranchId = async (tenantId) => {
    ...
    if (rows.length !== 1) {
        throw new HttpsError('failed-precondition', rows.length === 0
...
const loadOpenSessionForBranch = async (actor, branchId) => {
    const openSessions = await db.collection('repair_treasury_sessions')
        .where('branchId', '==', branchId)
        .where('status', '==', 'open')
        .limit(5)
...
async function submitTreasurySettlement(actor, data) {
    requirePermission(actor, ['repair.treasury.manage'], 'ليس لديك صلاحية إدارة خزينة الصيانة.');
    ...
    return { ok: true, ...result, countedAmount, toBranchId: main.id };
}
async function rejectTreasurySettlement(actor, data) {
    requirePermission(actor, ['repair.treasury.manage'], 'ليس لديك صلاحية إدارة خزينة الصيانة.');
    ...
    return { ok: true, settlementId, status: 'rejected' };
}
async function approveTreasurySettlement(actor, data) {
    requirePermission(actor, ['repair.treasury.manage'], 'ليس لديك صلاحية إدارة خزينة الصيانة.');
    requireAdminTreasury(actor);
    const settlementId = String(data.settlementId || '').trim();
    if (!settlementId)
        throw new HttpsError('invalid-argument', 'معرّف التسوية مطلوب.');
    const settlementRef = db.collection('repair_treasury_settlements').doc(settlementId);
    const initial = await settlementRef.get();
    if (!initial.exists || String(initial.data()?.tenantId || '') !== actor.tenantId) {
        throw new HttpsError('not-found', 'طلب التسوية غير موجود.');
    }
    const settlement = initial.data();
    ...
    return { ok: true, settlementId, status: 'approved', amount, ...result };
}
