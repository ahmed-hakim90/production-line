...
const hasPerm = (actor, keys) => actor.isSuperAdmin || keys.some((key) => actor.permissions[key] === true);
...
const stockDocId = (branchId, warehouseId, partId) => `${branchId}__${warehouseId}__${partId}`;
/**
 ...
 */
export const syncRepairCenterCatalogFromInventory = onCall({ region: 'us-central1', memory: '512MiB', timeoutSeconds: 120 }, async (request) => {
    ...
    const warehouseName = String(data.warehouseName || warehouseSnap.data()?.name || `مخزن ${branchSnap.data()?.name || branchId}`).trim();
    ...
    if (materialBalances.length === 0) {
        return { ok: true, createdParts: 0, synced: 0, failed: 0 };
    }
    ...
    for (const balance of materialBalances) {
        ...
        try {
            ...
            const stockRef = db.collection('repair_spare_parts_stock').doc(stockDocId(branchId, warehouseId, part.id));
    ...
    return { ok: true, createdParts, synced, failed };
});
