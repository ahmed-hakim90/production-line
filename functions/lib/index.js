...
import { countBostaDeliveriesCreatedInRange, listBostaDeliveriesCreatedInRange, parseYmdRangeToDispatchDayLocalBounds, } from './bostaHttp.js';
...
const hasOnlineDispatchBostaAccess = async (uid) => hasAnyPermission(uid, [
    'onlineDispatch.view',
...
function parseBostaHttpJsonBody(req) {
    try {
...
async function getUidFromBearerAuthorization(req) {
...
function sendBostaHttpError(res, err) {
...
async function bostaGetDeliveriesCreatedCountCore(requesterUid, rangeFrom, rangeTo) {
    ...
    if (!apiKey) {
        throw new HttpsError('failed-precondition', 'لم يُهيأ مفتاح بوسطة. نفّذ: firebase functions:secrets:set BOSTA_API_KEY (أو للمحاكي: BOSTA_API_KEY في .env).');
    }
...
async function bostaListDeliveriesForRangeCore(requesterUid, rangeFrom, rangeTo) {
    const allowed = await hasOnlineDispatchBostaAccess(requesterUid);
    if (!allowed) {
        throw new HttpsError('permission-denied', 'ليس لديك صلاحية عرض مؤشرات الأونلاين.');
    }
...
async function bostaSyncOnlineDispatchStatusesCore(requesterUid, limit, options) {
    const allowed = await hasOnlineDispatchBostaAccess(requesterUid);
    if (!allowed) {
...
async function bostaSyncOnlineDispatchByDocIdsCore(requesterUid, docIds) {
    const allowed = await hasOnlineDispatchBostaAccess(requesterUid);
    if (!allowed) {
        throw new HttpsError('permission-denied', 'ليس لديك صلاحية.');
    }
...
function resolveBostaApiKey() {
    const env = String(process.env.BOSTA_API_KEY ||
...
export const syncBostaOnlineDispatchStatuses = onCall({
    ...
    const advancePaginationCursor = Boolean(request.data?.advancePaginationCursor);
