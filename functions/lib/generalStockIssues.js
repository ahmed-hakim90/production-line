...
const qty = (value) => {
    ...
    if (!Number.isFinite(number) || number <= 0)
        throw new HttpsError('invalid-argument', 'كل كمية صرف يجب أن تكون أكبر من صفر.');
    return Math.round(number * 10000) / 10000;
};
export const postGeneralStockIssueHandler = async (request) => {
    ...
    if (!warehouseId || !PURPOSES.has(purpose))
        throw new HttpsError('invalid-argument', 'المخزن وغرض الصرف مطلوبان.');
    ...
    if (workOrderId) {
        ...
        if (productionStageId) {
            ...
            if (!stageSnap.exists || clean(stage?.tenantId) !== tenantId)
                throw new HttpsError('invalid-argument', 'مرحلة الإنتاج غير صالحة.');
    ...
    if (!rawLines.length || rawLines.length > 100)
        throw new HttpsError('invalid-argument', 'إذن الصرف يجب أن يحتوي من 1 إلى 100 بند.');
    const seen = new Set();
    const lines = rawLines.map((line) => {
        ...
        if (seen.has(key))
            throw new HttpsError('invalid-argument', 'لا يمكن تكرار نفس البند واللوكيشن في الإذن.');
        seen.add(key);
    ...
    await db.runTransaction(async (tx) => {
        ...
        const postedLines = lines.map((line, index) => {
            const balance = balanceSnaps[index].data();
        ...
        if (draftRef)
            tx.set(draftRef, { status: 'converted', postedVoucherId: voucherRef.id, postedReferenceNo: referenceNo, updatedAt: at }, { merge: true });
    });
    return { ok: true, id: voucherRef.id, referenceNo };
};
