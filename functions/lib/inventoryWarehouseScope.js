 ...
 * Unbound actors pass.
 */
export function assertActorWarehousesAllowed(boundWarehouseId, warehouseIds, message = 'هذا الحساب مرتبط بمخزن آخر.') {
    if (!boundWarehouseId)
        return;
    const ids = [
 ...
 */
export function assertActorWarehouseInvolved(boundWarehouseId, warehouseIds, message = 'هذا الحساب مرتبط بمخزن آخر.') {
    if (!boundWarehouseId)
        return;
    const ids = [
        ...new Set(warehouseIds
