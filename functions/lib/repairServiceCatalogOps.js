...
export const loadProtectedRepairServiceCatalog = async (tenantId) => {
    ...
    return { revision: 0, services: await legacyCatalog(tenantId), source: 'legacy' };
};
export const mutateRepairServiceCatalogHandler = async (request) => {
    ...
    if (operation !== 'save')
        throw new HttpsError('invalid-argument', 'عملية كتالوج غير مدعومة.');
    ...
    const revision = await db.runTransaction(async (tx) => {
        ...
        if (settingsSnap.exists) {
            ...
            const repairSettings = (settings.repairSettings || {});
            tx.set(settingsRef, {
                repairSettings: {
                    ...repairSettings,
    ...
    return { ok: true, revision, services };
};
