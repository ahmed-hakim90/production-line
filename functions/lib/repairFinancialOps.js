...
const assertBranchAccess = async (actor, branchId, branch) => {
    ...
    if (employeeIds.length > 0) {
        const snaps = await db.getAll(...employeeIds.map((id) => db.collection('employees').doc(id)));
        if (snaps.some((snap) => {
            const row = snap.data();
            return snap.exists
...
const isActorBranchManager = async (actor, branch) => {
...
const computeFinalCost = (job) => {
    ...
    const computed = partsCost
        ...
        + products.reduce((sum, raw) => sum + money(raw.finalCost), 0);
 ...
 */
export const deliverRepairJobAndCollectHandler = async (request) => {
    ...
    const configuredStatuses = settingsSnap.data()?.repairSettings?.workflow?.statuses;
    ...
    const deliverySourceStatus = String(readyStatus?.id
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const pendingParts = (Array.isArray(job.partsUsed) ? job.partsUsed : [])
            ...
            .includes(String(raw.fulfillmentStatus || '')));
        ...
        if (balanceDue > 0.00001) {
            ...
            if (!sessionSnap.exists)
                throw new HttpsError('failed-precondition', 'جلسة الخزينة غير موجودة.');
            const session = sessionSnap.data();
            if (String(session.tenantId || '').trim() !== actor.tenantId
        ...
        const deliveryAuthorizationNo = String(job.deliveryAuthorizationNo || `DEL-${String(job.receiptNo || jobId)}`);
        tx.update(jobRef, {
            status: 'delivered',
...
const normalizeStoredInvoiceLines = (raw) => {
...
const normalizeInvoiceLineRequests = (raw) => {
    if (!Array.isArray(raw) || raw.length === 0 || raw.length > 100) {
        throw new HttpsError('invalid-argument', 'أضف من بند واحد إلى 100 بند للفاتورة.');
    }
    return raw.map((value) => {
...
const resolveInvoiceLinesFromMaterials = async (actor, branchId, raw, customerType) => {
    ...
    for (const snap of partSnaps) {
        if (!snap.exists)
            throw new HttpsError('not-found', 'قطعة الغيار غير موجودة في كتالوج الفرع.');
        const row = snap.data();
    ...
    return requests.map((requestRow) => {
        ...
        if (!material)
            throw new HttpsError('failed-precondition', 'تعذر قراءة سعر الصنف من ماستر الخامات.');
        return {
...
const invoiceLineQuantities = (lines) => {
...
const getOpenTreasurySession = async (actor, branchId, required) => {
    if (!required)
        return null;
...
const formatInvoiceNo = (sequence) => `RSI-${String(sequence).padStart(5, '0')}`;
const stockDocId = (branchId, partId, warehouseId) => warehouseId ? `${branchId}__${warehouseId}__${partId}` : `${branchId}__${partId}`;
const invoiceDiscount = (gross, type, rawValue) => {
    const value = money(rawValue);
    if (type === 'percent' && value > 100)
        throw new HttpsError('invalid-argument', 'نسبة الخصم لا تتجاوز 100%.');
    const amount = type === 'amount'
        ? value
        : type === 'percent' ? Math.round(gross * value) / 100 : 0;
    if (amount > gross + 0.001)
...
const requireInvoiceAccounting = async (actor, branch, opts) => {
    ...
    if (missing.length)
        throw new HttpsError('failed-precondition', `أكمل ربط حسابات الفرع: ${missing.join(', ')}`);
...
export const mutateRepairSalesInvoiceHandler = async (request) => {
    ...
    if (!branchId)
        throw new HttpsError('invalid-argument', 'الفرع مطلوب.');
    ...
    if (!warehouseId)
        throw new HttpsError('failed-precondition', 'لا يوجد مخزن مرتبط بفرع الصيانة.');
    let invoiceCustomer = null;
    const requestedCustomerId = String(payload.customerId || '').trim();
    if (operation === 'prepare') {
        if (!requestedCustomerId)
            throw new HttpsError('invalid-argument', 'اختيار العميل مطلوب قبل حفظ الفاتورة.');
    ...
    if (operation === 'resolve_discount') {
        ...
        const result = await db.runTransaction(async (tx) => {
            ...
            const row = snap.data();
            if (!snap.exists || String(row?.tenantId || '') !== actor.tenantId)
                throw new HttpsError('not-found', 'الفاتورة غير موجودة.');
            if (String(row?.status || '') !== 'pending_discount_approval')
                throw new HttpsError('failed-precondition', 'لا يوجد خصم معلق لهذه الفاتورة.');
            if (String(row?.discountRequestedBy || '') === actor.uid)
                throw new HttpsError('permission-denied', 'لا يمكن لمقدم الخصم اعتماد طلبه.');
            const at = new Date().toISOString();
    ...
    if (operation === 'prepare') {
        const result = await db.runTransaction(async (tx) => {
            ...
            if (existingRef)
                tx.set(invoiceRef, docData, { merge: true });
            else {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        let session = null;
        let monthCloseSnap = null;
        let treasuryEntrySnap = null;
        if (sessionRef) {
            ...
            monthCloseSnap = await tx.get(db.collection('repair_treasury_month_closes').doc(`${actor.tenantId}_${branchId}_${month}`));
            treasuryEntrySnap = await tx.get(treasuryEntryRef);
        }
        if (monthCloseSnap?.exists && String(monthCloseSnap.data()?.status || '') === 'closed') {
            throw new HttpsError('failed-precondition', 'شهر جلسة الخزينة مقفول.');
        }
        const oldMap = invoiceLineQuantities(operation === 'post'
            ? []
        ...
        const partMetaByPartId = new Map();
        ...
        const inventoryPartIds = new Set(inventoryMovements.flatMap((row) => row.partIds));
        ...
        // Inventory SoT reads
        const invBalanceRefs = inventoryMovements.map((row) => db.collection(STOCK_ITEMS_COLLECTION).doc(stockItemsBalanceDocId(warehouseId, row.materialId)));
        ...
        for (let i = 0; i < inventoryMovements.length; i += 1) {
            ...
            if (movement.direction === 'OUT' && balQty - movement.quantity < -0.00001) {
                throw new HttpsError('failed-precondition', `الكمية غير كافية في مخزن المركز للصنف: ${movement.partName}`);
            }
        }
        const at = new Date().toISOString();
        ...
        for (const [partId, stock] of stockRows.entries()) {
            if (Math.abs(stock.delta) <= 0.00001)
                continue;
