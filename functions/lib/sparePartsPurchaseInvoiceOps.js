...
const balanceDocId = (warehouseId, materialId) => `${warehouseId}__material__${materialId}`;
const weightedAverage = (oldQty, oldAvg, recvQty, unitPrice) => {
    const q0 = Math.max(0, oldQty);
    const q1 = Math.max(0, recvQty);
    if (q1 <= 0)
        return roundMoney(oldAvg);
    if (q0 <= 0)
        return roundMoney(unitPrice);
    return roundMoney(((q0 * oldAvg) + (q1 * unitPrice)) / (q0 + q1));
};
const resolveCentralWarehouse = async (tenantId) => {
...
const normalizeLines = (raw) => {
    ...
    if (raw.length > 80)
        throw new HttpsError('invalid-argument', 'تجاوزت الحد الأقصى لبنود الفاتورة.');
    const out = [];
    const seen = new Set();
    for (const row of raw) {
        ...
        if (!materialId)
            throw new HttpsError('invalid-argument', 'معرّف الصنف مطلوب.');
        if (!(quantity > 0))
            throw new HttpsError('invalid-argument', 'كمية كل بند يجب أن تكون أكبر من صفر.');
        if (!(unitPrice >= 0))
            throw new HttpsError('invalid-argument', 'سعر الوحدة غير صالح.');
...
const assertSpareEligibleMaterial = (material, materialId) => {
    ...
    if (!['raw_material', 'consumable', 'material'].includes(type)) {
        throw new HttpsError('failed-precondition', `الصنف ${materialId} ليس من أنواع قطع/مستهلكات الصيانة المسموحة.`);
    }
};
export const mutateSparePartsPurchaseInvoiceHandler = async (request) => {
    const actor = await loadActor(requireAuth(request));
    requirePermission(actor, ['inventory.transactions.create', 'sparePartsReplenishment.prepare', 'repair.parts.manage'], 'ليس لديك صلاحية ترحيل فاتورة شراء قطع الغيار.');
    ...
    const materialSnaps = await Promise.all(linesIn.map((line) => db.collection('materials').doc(line.materialId).get()));
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        const balanceRefs = resolvedLines.map((line) => db.collection(STOCK_ITEMS).doc(balanceDocId(warehouse.id, line.materialId)));
    ...
    return { ok: true, operation, ...result, warehouseId: warehouse.id, total: grossAmount };
};
