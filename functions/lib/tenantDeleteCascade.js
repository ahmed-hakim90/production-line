...
async function deleteQueryInBatches(db, collName, tenantId) {
    let deleted = 0;
...
async function deleteCollectionPages(db, colRef) {
    let deleted = 0;
    // eslint-disable-next-line no-constant-condition
    while (true) {
...
export async function deleteTenantCascade(db, tenantId, slug) {
    let deletedFirestoreDocs = 0;
    const usersSnap = await db.collection(USERS_COLLECTION).where('tenantId', '==', tenantId).get();
    const uids = usersSnap.docs.map((d) => d.id);
    for (const uid of uids) {
        deletedFirestoreDocs += await deleteCollectionPages(db, db.collection(USERS_COLLECTION).doc(uid).collection('preferences'));
        deletedFirestoreDocs += await deleteCollectionPages(db, db.collection(USERS_COLLECTION).doc(uid).collection('fcmTokens'));
