...
export async function readWorkOrderCycle(uid, input = {}) {
    ...
    const orders = await Promise.all(rows.map(async ({ id, data }) => {
        ...
        return { ...clean(data), id, inspectorUids: lineAssignment.data()?.inspectorUids || [], slots: slots.docs.map(slot => ({ ...clean(slot.data()), id: slot.id })).sort((a, b) => String(a.date + a.startTime).localeCompare(String(b.date + b.startTime))) };
    }));
    ...
    if (input.directory) {
        const basic = async (collection) => {
            ...
            if (snap.size > 500)
                throw new HttpsError('resource-exhausted', 'دليل الاختيار تجاوز حد المختبر؛ يحتاج بحثًا مقسمًا.');
        ...
        if (permissions['workOrders.create'] === true || permissions['workOrders.assignInspectors'] === true) {
            ...
            if (users.size > 500 || roles.size > 500)
                throw new HttpsError('resource-exhausted', 'دليل المستخدمين تجاوز حد المختبر.');
            ...
            for (const doc of users.docs) {
                ...
                if (grants['workOrders.execute'] === true && permissions['workOrders.create'] === true)
