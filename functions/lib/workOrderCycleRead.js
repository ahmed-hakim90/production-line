...
const clean = (data) => Object.fromEntries(Object.entries(data).map(([key, value]) => [key, value?.toDate instanceof Function ? value.toDate().toISOString() : value]));
/** Bounded, tenant/assignment-scoped reads; directories never expose HR/payroll or auth fields. */
export async function readWorkOrderCycle(uid, input = {}) {
    requireWorkOrderLab();
    if (!uid)
        throw new HttpsError('unauthenticated', 'يجب تسجيل الدخول.');
    const db = getDb();
    const user = (await db.doc(`users/${uid}`).get()).data();
    if (user?.isActive !== true || !user.roleId || !user.tenantId)
        throw denied();
    ...
    if (!manager && !supervisor && !inspector && !packaging)
        throw denied();
    ...
    let truncated = false;
    if (input.orderId) {
        if (!/^[\w-]{1,120}$/.test(input.orderId))
            throw new HttpsError('invalid-argument', 'هوية الأمر غير صالحة.');
        const snap = await db.collection('work_orders').doc(input.orderId).get();
        const data = snap.data();
        if (!data || !visible(data))
            throw denied();
    ...
    const orders = await Promise.all(rows.map(async ({ id, data }) => {
        ...
        const supervisorUidsToResolve = new Set();
        for (const event of auditEvents) {
            if (event.previousSupervisorUid)
                supervisorUidsToResolve.add(event.previousSupervisorUid);
            if (event.supervisorUid)
                supervisorUidsToResolve.add(event.supervisorUid);
        }
    ...
    if (input.directory) {
        ...
        const basic = async (collection) => {
            const snap = await db.collection(collection).where('tenantId', '==', tenantId).limit(DIRECTORY_LIMIT + 1).get();
            if (snap.size > DIRECTORY_LIMIT)
                throw new HttpsError('resource-exhausted', 'دليل الاختيار كبير جدًا؛ يحتاج بحثًا مقسمًا (لم يُبنَ بعد).');
        ...
        if (permissions['workOrders.create'] === true)
            [directory.products, directory.lines] = await Promise.all([basic('products'), basic('production_lines')]);
        if (supervisor) {
        ...
        if (permissions['workOrders.create'] === true || permissions['workOrders.approve'] === true || permissions['workOrders.assignInspectors'] === true) {
            ...
            if (users.size > DIRECTORY_LIMIT || roles.size > DIRECTORY_LIMIT)
                throw new HttpsError('resource-exhausted', 'دليل المستخدمين كبير جدًا؛ يحتاج بحثًا مقسمًا (لم يُبنَ بعد).');
            const roleMap = new Map(roles.docs.map(doc => [doc.id, doc.data().permissions || {}]));
            for (const doc of users.docs) {
                const person = doc.data();
                const grants = roleMap.get(person.roleId) || {};
                if (person.isActive !== true)
                    continue;
                const entry = { id: doc.id, name: String(person.displayName || doc.id) };
                if (grants['workOrders.execute'] === true && (permissions['workOrders.create'] === true || permissions['workOrders.approve'] === true))
                    directory.supervisors.push(entry);
                if (grants['workOrders.inspect'] === true && permissions['workOrders.assignInspectors'] === true)
                    directory.inspectors.push(entry);
            }
        }
    }
