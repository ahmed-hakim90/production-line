...
const positiveInt = (value, name = 'الكمية') => {
    const n = Math.round(Number(value));
    if (!Number.isFinite(n) || n <= 0)
        throw new HttpsError('invalid-argument', `${name} يجب أن تكون أكبر من صفر.`);
    return n;
};
const nonNegativeInt = (value, name = 'الكمية') => {
    const n = Math.round(Number(value));
    if (!Number.isFinite(n) || n < 0)
        throw new HttpsError('invalid-argument', `${name} غير صالحة.`);
    return n;
};
const hash = (value) => createHash('sha256').update(value).digest('hex');
const safeId = (value) => value.replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 80);
const balanceId = (warehouseId, productId) => `${warehouseId}__finished_good__${productId}`;
const custodyId = (jobId, itemId) => `${jobId}__${safeId(itemId)}`;
function pepper() {
    const value = String(process.env.CUSTOMER_PORTAL_PIN_PEPPER || '').trim();
    if (!value)
        throw new HttpsError('failed-precondition', 'سر تشفير بوابة العملاء غير مضبوط.');
    return value;
}
...
function requirePermission(actor, permissions, message) {
    if (actor.isSuperAdmin || permissions.some((key) => actor.permissions[key] === true))
        return;
    throw new HttpsError('permission-denied', message);
}
function requireBranch(actor, branchId, allowAll = false) {
    if (actor.isSuperAdmin || actor.permissions['repair.callCenter.viewAll'] === true || allowAll)
        return;
...
export async function ensureRepairCustomerWarehouses(tenantId, branchId) {
    ...
    if (!branchSnap.exists)
        throw new HttpsError('not-found', 'مركز الصيانة غير موجود.');
...
function stockWrite(tx, ref, current, delta, meta) {
    const next = current + delta;
    if (next < 0)
        throw new HttpsError('failed-precondition', 'رصيد عهدة المركز غير كافٍ.');
    tx.set(ref, {
        ...meta,
        quantity: next,
        reservedQty: 0,
...
async function postJobCustody(jobId) {
    const jobRef = db.collection(JOBS).doc(jobId);
    const jobSnap = await jobRef.get();
    if (!jobSnap.exists)
        return { posted: 0 };
    const job = jobSnap.data();
    if (job.custodyPostedAt)
        return { posted: 0 };
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!freshJob.exists || freshJob.data()?.custodyPostedAt)
            return;
...
async function createDirectRepairJob(actor, data) {
    ...
    const incoming = (data.job && typeof data.job === 'object' ? data.job : {});
    const branchId = text(incoming.branchId, 128);
    const receiptNo = text(incoming.receiptNo, 80);
    if (!jobId || !branchId || !receiptNo)
        throw new HttpsError('invalid-argument', 'بيانات طلب الصيانة غير مكتملة.');
    requireBranch(actor, branchId);
    const warehouses = await ensureRepairCustomerWarehouses(actor.tenantId, branchId);
    const rawProducts = (Array.isArray(incoming.jobProducts) ? incoming.jobProducts : []);
    if (!rawProducts.length || rawProducts.length > 50)
        throw new HttpsError('invalid-argument', 'أضف منتجًا واحدًا على الأقل.');
    const products = rawProducts.map((row, index) => {
        const productId = text(row.productId, 128);
        if (!productId)
            throw new HttpsError('invalid-argument', `منتج السطر ${index + 1} غير صالح.`);
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (existingJob.exists) {
            if (text(existingJob.data()?.tenantId, 128) !== actor.tenantId)
                throw new HttpsError('already-exists', 'معرف الطلب مستخدم.');
            alreadyCreated = true;
            return;
        }
    ...
    return { ok: true, jobId, receiptNo, alreadyCreated };
}
export const onRepairJobCreatedCustody = onDocumentCreated({ document: 'repair_jobs/{jobId}', region: 'us-central1', retry: true }, async (event) => { await postJobCustody(event.params.jobId); });
...
async function sessionContext(request) {
    const token = text(request.data?.sessionToken, 256);
    if (token.length < 40)
        throw new HttpsError('unauthenticated', 'جلسة العميل غير صالحة.');
...
export async function customerPortalLoginHandler(request) {
    ...
    if (!tenantSlug || !customerCode || !new RegExp(`^\\d{${CUSTOMER_PORTAL_PIN_DIGITS}}$`).test(pin))
        throw new HttpsError('invalid-argument', 'بيانات الدخول غير صالحة.');
    ...
    if (!tenantSnap.exists || text(tenantSnap.data()?.status, 40) !== 'active')
        throw new HttpsError('permission-denied', 'الشركة غير متاحة.');
    ...
    if (Number(limitData?.lockedUntilMs || 0) > Date.now())
        throw new HttpsError('resource-exhausted', 'محاولات كثيرة. حاول بعد 15 دقيقة.');
    ...
    return { ok: true, sessionToken: rawToken, expiresAtMs };
}
export async function getCustomerPortalHomeHandler(request) {
    ...
    const publicJob = (doc) => {
        ...
        return { id: doc.id, receiptNo: text(row.receiptNo, 80), status: text(row.status, 80), branchId: text(row.branchId, 128),
            ...
            products: (Array.isArray(row.jobProducts) ? row.jobProducts : []).map((p) => ({
...
export async function lookupPortalProductHandler(request) {
    const ctx = await sessionContext(request);
    const barcode = text(request.data?.barcode, 120);
    if (!barcode)
        throw new HttpsError('invalid-argument', 'الباركود مطلوب.');
    ...
    if (snap.empty)
        snap = await db.collection('products').where('tenantId', '==', ctx.tenantId).where('barcode', '==', barcode).limit(1).get();
    const product = snap.docs[0];
    if (!product)
        throw new HttpsError('not-found', 'الباركود غير مسجل على أي منتج.');
    const row = product.data();
    return { ok: true, product: { id: product.id, name: text(row.name, 160), code: text(row.code, 80), barcode } };
}
export async function createCustomerServiceRequestHandler(request) {
    const ctx = await sessionContext(request);
    const incoming = Array.isArray(request.data?.lines) ? request.data.lines : [];
    ...
    for (let index = 0; index < normalizedIncoming.length; index += 1) {
        ...
        if (productSnap.empty)
            productSnap = await db.collection('products').where('tenantId', '==', ctx.tenantId)
                .where('barcode', '==', barcode).limit(1).get();
        const product = productSnap.docs[0];
        if (!product)
            throw new HttpsError('not-found', `الباركود في السطر ${index + 1} غير مسجل.`);
        const row = product.data();
    ...
    return { ok: true, requestId: ref.id, requestNo };
}
async function generatePin(actor, data) {
...
async function assignRequest(actor, data) {
    ...
    if (!requestSnap.exists || text(requestSnap.data()?.tenantId, 128) !== actor.tenantId)
        throw new HttpsError('not-found', 'طلب العميل غير موجود.');
    if (!branchSnap.exists || text(branchSnap.data()?.tenantId, 128) !== actor.tenantId)
        throw new HttpsError('not-found', 'المركز غير موجود.');
    const requestRow = requestSnap.data();
    if (text(requestRow.status, 40) === 'converted')
        throw new HttpsError('failed-precondition', 'تم تحويل الطلب بالفعل.');
...
async function receiveRequest(actor, data) {
    ...
    requireBranch(actor, branchId);
    if (text(requestRow.status, 40) !== 'assigned')
        throw new HttpsError('failed-precondition', 'الطلب غير متاح للاستلام.');
    const warehouses = await ensureRepairCustomerWarehouses(actor.tenantId, branchId);
    const submitted = (Array.isArray(data.lines) ? data.lines : []);
    const byLine = new Map(submitted.map((line) => [text(line.lineId, 100), line]));
    const requestLines = (Array.isArray(requestRow.lines) ? requestRow.lines : []);
    const actualLines = requestLines.map((line) => {
        ...
        if (receivedQuantity !== requestedQuantity && !differenceNote)
            throw new HttpsError('invalid-argument', 'ملاحظة اختلاف الكمية مطلوبة.');
        return { ...line, requestedQuantity, receivedQuantity, differenceNote };
    });
    if (!actualLines.some((line) => Number(line.receivedQuantity) > 0))
        throw new HttpsError('invalid-argument', 'يجب استلام وحدة واحدة على الأقل.');
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!freshRequest.exists || freshRequest.data()?.status !== 'assigned')
            throw new HttpsError('already-exists', 'تم استلام الطلب من قبل.');
        const seq = Number(counter.data()?.value || 0) + 1;
    ...
    return { ok: true, jobId: jobRef.id, receiptNo };
}
async function recordUnrepairable(actor, data, options = {}) {
    ...
    const job = jobSnap.data();
    const branchId = text(job.branchId, 128);
    requireBranch(actor, branchId, options.allowCrossBranch === true || actor.permissions['repair.custody.correct'] === true);
    const warehouses = await ensureRepairCustomerWarehouses(actor.tenantId, branchId);
    const products = (Array.isArray(job.jobProducts) ? job.jobProducts : []);
    const index = products.findIndex((row) => text(row.itemId, 100) === itemId);
    if (index < 0)
        throw new HttpsError('not-found', 'منتج الطلب غير موجود.');
    const line = products[index];
    const totalQty = Math.max(1, Number(line.receivedQuantity || line.quantity || 1));
    if (nextQty > totalQty)
        throw new HttpsError('invalid-argument', 'الكمية غير القابلة للإصلاح أكبر من المستلم.');
    ...
    if (!recordBefore.exists)
        throw new HttpsError('failed-precondition', 'سجل عهدة المنتج غير موجود.');
    ...
    if (delta > 0) {
        if (reasonCode) {
            ...
            if (!selectedReason)
                throw new HttpsError('invalid-argument', 'اختر سببًا معتمدًا لعدم قابلية الإصلاح.');
    ...
    if (delta !== 0) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const freshProducts = (Array.isArray(freshJob.data()?.jobProducts) ? freshJob.data()?.jobProducts : []);
            ...
            if (freshOld !== oldQty)
                throw new HttpsError('aborted', 'تم تعديل السطر من مستخدم آخر.');
            const recordRow = (record.data() || {});
            ...
            const previousDecisionQuantity = Math.max(Number(freshProducts[freshIndex].unrepairableDecisionQuantity || 0), freshOld);
...
export async function recordAssignedJobFullyUnrepairable(input) {
    ...
    const products = (Array.isArray(jobSnap.data()?.jobProducts) ? jobSnap.data()?.jobProducts : []);
...
async function reopenUnrepairable(actor, data) {
    ...
    if (!recordSnap.exists)
        throw new HttpsError('failed-precondition', 'سجل مخزن غير القابل للإصلاح غير موجود.');
    ...
    const products = (Array.isArray(fresh.data()?.jobProducts) ? fresh.data()?.jobProducts : []);
    const index = products.findIndex((row) => text(row.itemId, 100) === itemId);
    if (index >= 0) {
        products[index] = {
            ...products[index],
    ...
    const statusHistory = Array.isArray(fresh.data()?.statusHistory) ? [...fresh.data()?.statusHistory] : [];
    ...
    return { ok: true, status: 'repairing', quantity };
}
async function handover(actor, data) {
    ...
    if (!allowed) {
        throw new HttpsError('failed-precondition', 'خروج الجهاز من العهدة متاح بعد التسليم المالي المعتمد أو إلغاء الطلب فقط.');
    }
    const branchId = text(job.branchId, 128);
    requireBranch(actor, branchId);
    const warehouses = await ensureRepairCustomerWarehouses(actor.tenantId, branchId);
    ...
    await db.runTransaction(async (tx) => {
        ...
        if (!record.exists)
            throw new HttpsError('failed-precondition', 'سجل العهدة غير موجود.');
        ...
        if (quantity > available)
            throw new HttpsError('failed-precondition', 'الكمية المطلوب تسليمها أكبر من العهدة المتاحة.');
...
async function listComplaintJobOptions(actor) {
    ...
    const jobs = snap.docs.map((doc) => {
        const row = doc.data();
        const products = (Array.isArray(row.jobProducts) ? row.jobProducts : []);
        return {
            id: doc.id,
...
async function createRepairComplaint(actor, data) {
    ...
    if (!jobId)
        throw new HttpsError('invalid-argument', 'اختر طلب الصيانة المرتبط بالشكوى.');
    if (!subject)
        throw new HttpsError('invalid-argument', 'أدخل موضوع الشكوى.');
    const jobRef = db.collection(JOBS).doc(jobId);
    const jobSnap = await jobRef.get();
    if (!jobSnap.exists || text(jobSnap.data()?.tenantId, 128) !== actor.tenantId) {
        throw new HttpsError('not-found', 'طلب الصيانة غير موجود.');
...
function callCenterJobPayload(id, row) {
    const products = (Array.isArray(row.jobProducts) ? row.jobProducts : []);
    return {
        id,
...
async function listCallCenterJobs(actor, data) {
    ...
    const jobs = snap.docs
        ...
        .map((doc) => callCenterJobPayload(doc.id, doc.data()))
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
        .slice(0, 500);
    return { ok: true, jobs };
}
async function listRepairFollowUps(actor, data) {
...
async function createRepairFollowUp(actor, data) {
    ...
    if (!note)
        throw new HttpsError('invalid-argument', 'اكتب ملاحظة المتابعة.');
    const jobRef = db.collection(JOBS).doc(jobId);
    const jobSnap = await jobRef.get();
    if (!jobSnap.exists || text(jobSnap.data()?.tenantId, 128) !== actor.tenantId) {
        throw new HttpsError('not-found', 'طلب الصيانة غير موجود.');
    ...
    await batch.commit();
    return { ok: true, followUpId: followUpRef.id };
}
async function mutateReplacement(actor, data) {
    ...
    if (action === 'createReplacement') {
        ...
        const branchId = text(job.branchId, 128);
        requireBranch(actor, branchId);
        const line = (Array.isArray(job.jobProducts) ? job.jobProducts : []).find((row) => text(row.itemId, 100) === itemId);
        if (!line)
            throw new HttpsError('not-found', 'منتج الطلب غير موجود.');
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (used + quantity > Number(line.unrepairableQuantity || 0))
                throw new HttpsError('failed-precondition', 'الكمية تتجاوز غير القابل للإصلاح المتاح.');
    ...
    if (action === 'approveReplacement') {
        ...
        if (!product.exists || text(product.data()?.tenantId, 128) !== actor.tenantId)
            throw new HttpsError('not-found', 'المنتج البديل غير موجود.');
        const quantity = positiveInt(data.quantity);
        if (quantity > Number(row.requestedQuantity || 0))
            throw new HttpsError('invalid-argument', 'الكمية المعتمدة أكبر من المطلوبة.');
        await db.runTransaction(async (tx) => {
            const fresh = await tx.get(ref);
            if (text(fresh.data()?.status, 40) !== 'pending_approval')
                throw new HttpsError('failed-precondition', 'طلب الاستبدال ليس قيد الاعتماد.');
    ...
    if (action === 'deliverReplacement') {
        ...
        await db.runTransaction(async (tx) => {
            const fresh = await tx.get(ref);
            if (text(fresh.data()?.status, 40) !== 'approved')
                throw new HttpsError('failed-precondition', 'طلب الاستبدال غير معتمد.');
    ...
    if (action === 'rejectReplacement' || action === 'cancelReplacement') {
        ...
        await db.runTransaction(async (tx) => {
            ...
            if (action === 'rejectReplacement' && currentStatus !== 'pending_approval')
                throw new HttpsError('failed-precondition', 'لا يمكن رفض الطلب بحالته الحالية.');
            if (action === 'cancelReplacement' && !['pending_approval', 'approved'].includes(currentStatus))
                throw new HttpsError('failed-precondition', 'لا يمكن إلغاء الطلب بحالته الحالية.');
...
export async function mutateRepairCustomerOpsHandler(request) {
    ...
    if (action === 'ensureWarehouses') {
        ...
        const branchId = text(data.branchId, 128);
        return { ok: true, ...(await ensureRepairCustomerWarehouses(actor.tenantId, branchId)) };
    }
    if (action === 'backfillCustomerCustody') {
        ...
        if (cursor)
            jobsQuery = jobsQuery.startAfter(cursor);
        ...
        const productCatalogById = new Map(productsSnap.docs.map((product) => [product.id, product.data()]));
        for (const branch of branchesSnap.docs)
            await ensureRepairCustomerWarehouses(actor.tenantId, branch.id);
        let custodyJobs = 0;
        ...
        for (const jobDoc of jobsSnap.docs) {
            ...
            const jobProducts = (Array.isArray(job.jobProducts) ? job.jobProducts : []);
            ...
            if (status === 'unrepairable') {
                const migrationActor = { ...actor, isSuperAdmin: true };
                const products = (Array.isArray(job.jobProducts) ? job.jobProducts : []);
                for (let index = 0; index < products.length; index += 1) {
                    const line = products[index];
                    const itemId = text(line.itemId || `item-${index + 1}`, 100);
                    ...
                    await recordUnrepairable(migrationActor, { jobId: jobDoc.id, itemId, quantity: qty,
                        reasonNote: text(job.closedReason, 1000) || 'ترحيل طلب قديم غير قابل للإصلاح' }, { allowLegacyReason: true });
                }
                unrepairableJobs += 1;
            }
        }
        let barcodeClaims = 0;
        const preparedBarcodes = new Map();
        ...
        if (batchSize)
            productBatches.push(batch);
        for (const writeBatch of productBatches)
            await writeBatch.commit();
        return { ok: true, branches: branchesSnap.size, custodyJobs, unrepairableJobs,
    ...
    if (action === 'postCustody') {
        ...
        return { ok: true, ...(await postJobCustody(text(data.jobId, 128))) };
    }
    if (action === 'createRepairJobWithCustody')
        return createDirectRepairJob(actor, data);
    if (action === 'assignRequest')
        return assignRequest(actor, data);
    if (action === 'receiveRequest')
        return receiveRequest(actor, data);
    if (action === 'recordUnrepairable')
        return recordUnrepairable(actor, data);
    if (action === 'reopenUnrepairable')
        return reopenUnrepairable(actor, data);
    if (action === 'handover')
        return handover(actor, data);
    if (action === 'listComplaintJobOptions')
        return listComplaintJobOptions(actor);
    if (action === 'createRepairComplaint')
        return createRepairComplaint(actor, data);
    if (action === 'listCallCenterJobs')
        return listCallCenterJobs(actor, data);
    if (action === 'listRepairFollowUps')
        return listRepairFollowUps(actor, data);
    if (action === 'createRepairFollowUp')
        return createRepairFollowUp(actor, data);
    if (action.includes('Replacement'))
        return mutateReplacement(actor, data);
