...
function id(value) {
    if (typeof value !== 'string' || !/^[\w-]{1,120}$/.test(value))
        throw new HttpsError('invalid-argument', 'هوية غير صالحة.');
    return value;
}
...
function reason(value) {
    if (typeof value !== 'string' || !value.trim() || value.length > 2000)
        fail('السبب مطلوب، بحد أقصى ٢٠٠٠ حرف.');
    return value.trim();
}
...
export async function executeWorkOrderCycle(uid, input) {
    requireWorkOrderLab();
    if (!uid)
        throw new HttpsError('unauthenticated', 'يجب تسجيل الدخول.');
    if (!input || !Object.hasOwn(permissions, input.action))
    ...
    if (typeof payload !== 'object' || Array.isArray(payload))
        throw new HttpsError('invalid-argument', 'بيانات الإجراء غير صالحة.');
    const encoded = canonical({ ...input, payload });
    if (encoded.length > 100_000)
        throw new HttpsError('invalid-argument', 'حجم الطلب أكبر من المسموح.');
    ...
    return db.runTransaction(async (tx) => {
        ...
        if (user?.isActive !== true || !user.tenantId || !user.roleId)
        ...
        if (receipt) {
            if (receipt.uid !== uid || receipt.tenantId !== tenantId || receipt.fingerprint !== fingerprint)
                throw new HttpsError('already-exists', 'رقم الطلب مستخدم لإجراء مختلف.');
        ...
        if (input.action === 'prepare' || input.action === 'editDraft') {
            ...
            if (product?.tenantId !== tenantId || product.isActive === false || line?.tenantId !== tenantId || line.isActive === false || supervisor?.tenantId !== tenantId || supervisor.isActive !== true)
                fail('المنتج أو الخط أو المشرف خارج المصنع أو غير صالح.');
            ...
            if (!Array.isArray(payload.slots) || !payload.slots.length || payload.slots.length > 240)
                fail('خطة الساعات مطلوبة بحد أقصى ٢٤٠ ساعة.');
            const slots = payload.slots.map((slot, index) => {
                const date = String(slot.date || '');
                const startTime = String(slot.startTime || '');
                const endTime = String(slot.endTime || '');
                const parsedDate = new Date(date + 'T00:00:00Z');
                if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !Number.isFinite(parsedDate.getTime()) || parsedDate.toISOString().slice(0, 10) !== date || !/^([01]\d|2[0-3]):[0-5]\d$/.test(startTime) || !/^([01]\d|2[0-3]):[0-5]\d$/.test(endTime) || startTime >= endTime)
                    fail('توقيت الساعة غير صالح؛ عبور منتصف الليل غير مدعوم بعد.');
            ...
            for (let i = 1; i < slots.length; i++) {
                if (slots[i].date + slots[i].startTime < slots[i - 1].date + slots[i - 1].endTime)
                    fail('الساعات متداخلة أو غير مرتبة.');
            }
            ...
            if (input.action === 'reassignSupervisor') {
                ...
                for (const inspectorUid of inspectorUids) {
                    ...
                    if (inspector?.tenantId !== tenantId || inspector.isActive !== true || !inspector.roleId)
                        fail('مراقب غير صالح.');
                    const inspectorRole = (await tx.get(db.collection('roles').doc(String(inspector.roleId)))).data();
                    if (inspectorRole?.tenantId !== tenantId || inspectorRole.permissions?.['workOrders.inspect'] !== true)
                        fail('المستخدم ليس مراقب جودة.');
                }
                ...
                if (!['approved', 'in_progress'].includes(order.productionStatus))
                    fail('الإنتاج غير معتمد أو مقفول.');
                if (input.action === 'assignWorkers') {
                    const hasIds = payload.workerIds !== undefined;
                    const hasCount = payload.workerCount !== undefined;
                    if (hasIds === hasCount)
                        fail('حدد تكليف العمالة إما بالأسماء أو بالعدد، وليس الاثنين معًا.');
                    if (hasIds) {
                        const workerIds = ids(payload.workerIds);
                        for (const workerId of workerIds) {
                            const worker = (await tx.get(db.collection('employees').doc(workerId))).data();
                            if (worker?.tenantId !== tenantId || worker.isActive !== true)
                                fail('عامل غير نشط أو خارج المصنع.');
                        }
                        Object.assign(patch, { workerIds, workerCount: FieldValue.delete() });
                    }
                    else {
                        const workerCount = Number(payload.workerCount);
                        if (!Number.isInteger(workerCount) || workerCount <= 0 || workerCount > 500)
                            fail('عدد العمالة غير صالح.');
                    ...
                    if (input.action === 'start') {
                        if (order.qualityHold || order.activeSlotId || lineState?.activeOrderId || slot.status !== 'planned')
                            fail('يوجد قفل أو ساعة جارية على الخط.');
                        ...
                        if (order.activeSlotId !== slotId || lineState?.activeOrderId !== orderId || lineState.activeSlotId !== slotId)
                            fail('الساعة ليست الجارية.');
                        if (input.action === 'submit') {
                            if (slot.status !== 'open' || order.qualityHold)
                                fail('الساعة متوقفة.');
                            const actualQuantity = quantity(payload.actualQuantity);
                            const rejectedQuantity = quantity(payload.rejectedQuantity);
                            if (rejectedQuantity > actualQuantity)
                                fail('المرفوض يتجاوز المنتج.');
                            const notes = typeof payload.notes === 'string' ? payload.notes.trim() : '';
                            if (notes.length > 2000)
                                fail('الملاحظات بحد أقصى ٢٠٠٠ حرف.');
                            ...
                            if (slot.status !== 'paused' || order.qualityHold || !order.activeStopId)
                                fail('لا يمكن استئناف الساعة.');
