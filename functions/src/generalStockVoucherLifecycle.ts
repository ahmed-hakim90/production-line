...
async function assertWarehouse(warehouseId: string, tenantId: string, assignedWarehouseId: string, isSuperAdmin: boolean) {
  ...
  if (!isSuperAdmin && assignedWarehouseId && assignedWarehouseId !== warehouseId) throw new HttpsError('permission-denied', 'المخزن لا يطابق المخزن المرتبط بحسابك.');
  return snap;
}
...
async function voidVoucher(request: CallableRequest, kind: 'issue' | 'receipt') {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!current.isSuperAdmin && current.assignedWarehouseId && current.assignedWarehouseId !== warehouseId) throw new HttpsError('permission-denied', 'المخزن لا يطابق المخزن المرتبط بحسابك.');
