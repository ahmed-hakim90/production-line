...
export type AccountingAutomationFlag =
  | "autoPostRepairPayments"
  | "autoPostRepairSales"
  | "autoPostRepairCogs"
  | "autoPostRepairTreasury"
  | "autoPostInventory";

export type AccountingPostingDecision = {
  enabled: boolean;
  period: string;
  reason: "enabled" | "automation_disabled" | "opening_balance_pending" | "period_closed";
};
...
export async function accountingPostingDecision(
  tenantId: string,
  flag: AccountingAutomationFlag,
  dateIso: string,
): Promise<AccountingPostingDecision> {
  ...
  if (period < cutoverPeriod) return { enabled: true, period, reason: "enabled" };
...
export function queuePendingAccounting(
  tx: Transaction,
  input: {
    tenantId: string;
    source: string;
    sourceId: string;
    date: string;
    reason: AccountingPostingDecision["reason"];
    branchId?: string;
    costCenterId?: string;
    amount?: number;
