...
import {
  issueProductionIssueOrderForActor,
  loadProductionIssueActor,
  productionIssueActorHasPermission,
  type ActorContext,
} from './productionIssueStock.js';
...
type SourceType = 'work_order' | 'production_plan';
type InventoryItemType = 'material' | 'raw_material';
...
type ProductionIssueLine = CatalogLine & {
  qtyPerUnit: number;
...
type IssueOrder = {
  ...
  tenantId?: string;
  referenceNo: string;
  sourceType: SourceType | 'production_report';
  workOrderId?: string;
  ...
  status: string;
  origin?: 'production_request' | 'warehouse';
...
const tenantMatchesOrLegacy = (value: unknown, tenantId: string) => {
...
function assertAllowedKeys(data: Record<string, unknown>, allowed: string[]) {
  const unexpected = Object.keys(data).find((key) => !allowed.includes(key));
  if (unexpected) throw new HttpsError('invalid-argument', `حقل غير مسموح: ${unexpected}`);
}

function assertCreatePermission(actor: ActorContext) {
...
function assertApprovePermission(actor: ActorContext) {
  if (productionIssueActorHasPermission(actor, [
    'productionIssue.approve',
    'inventory.transfers.approve',
    'inventory.transactions.create',
...
function assertWarehouseScope(actor: ActorContext, warehouseId: string) {
  if (!actor.boundWarehouseId || actor.boundWarehouseId === warehouseId) return;
  throw new HttpsError('permission-denied', 'مستخدم المخزن يستطيع الصرف من مخزنه المرتبط فقط.');
}

async function requireTenantDoc(
  ...
  if (!snap.exists) throw new HttpsError('not-found', missingMessage);
...
function sourceQuantity(source: Source, sourceType: SourceType): number {
  if (sourceType === 'work_order') return num(source.quantity);
...
async function loadSource(
  actor: ActorContext,
  input: { workOrderId?: string; productionPlanId?: string },
): Promise<{ sourceType: SourceType; source: Source }> {
  ...
  const sourceType: SourceType = workOrderId ? 'work_order' : 'production_plan';
  const id = workOrderId || productionPlanId;
  const snap = await requireTenantDoc(
    sourceType === 'work_order' ? COLLECTIONS.workOrders : COLLECTIONS.plans,
    id,
    actor.tenantId,
    sourceType === 'work_order' ? 'أمر الشغل غير موجود.' : 'خطة الإنتاج غير موجودة.',
  );
...
async function loadWarehouse(actor: ActorContext, warehouseId: string) {
  assertWarehouseScope(actor, warehouseId);
  const snap = await requireTenantDoc(
    COLLECTIONS.warehouses,
    warehouseId,
    actor.tenantId,
    'مخزن صرف المكونات غير موجود.',
  );
  const data = snap.data() || {};
  if (data.isActive === false) throw new HttpsError('failed-precondition', 'مخزن صرف المكونات غير نشط.');
  return { id: snap.id, name: clean(data.name) || snap.id };
}

async function loadFloorWarehouse(actor: ActorContext, sourceWarehouseId: string) {
  const settingsSnap = await db.collection(COLLECTIONS.settings).doc(actor.tenantId).get();
  const routing = resolveInventoryRoutingFromSettings((settingsSnap.data() || {}) as {
  ...
  const snap = await requireTenantDoc(
    ...
    floorId,
    actor.tenantId,
    'مخزن صالة الإنتاج غير موجود.',
  );
  const data = snap.data() || {};
  if (data.isActive === false) throw new HttpsError('failed-precondition', 'مخزن صالة الإنتاج غير نشط.');
  return { id: snap.id, name: clean(data.name) || 'صالة الإنتاج' };
}

async function loadBomLines(tenantId: string, productId: string): Promise<BomLine[]> {
  const bomSnap = await db.collection(COLLECTIONS.boms)
    ...
    .where('status', '==', 'active')
    .get();
  const activeBom = [...bomSnap.docs].sort(
    (a, b) => num(b.data().version) - num(a.data().version),
  )[0];
  if (activeBom) {
...
async function loadCatalogLines(
  tenantId: string,
  bomLines: BomLine[],
): Promise<Map<string, CatalogLine>> {
  ...
  const materialSnaps = ids.length
    ...
    : [];
  const result = new Map<string, CatalogLine>();
  for (const snap of materialSnaps) {
    if (!snap.exists || !tenantMatchesOrLegacy(snap.data()?.tenantId, tenantId)) continue;
    const data = snap.data() || {};
    result.set(snap.id, {
      materialId: snap.id,
      itemType: 'material',
  ...
  if (result.size !== ids.length) {
    ...
    for (const line of bomLines) {
      if (result.has(line.itemId)) continue;
      ...
      if (!raw) {
        throw new HttpsError(
          'failed-precondition',
          `مكوّن BOM مرتبط بخامة محذوفة أو غير موجودة: ${line.itemName || line.itemId} (${line.itemId}).`,
          { reason: 'missing-bom-material', materialId: line.itemId, materialName: line.itemName },
        );
      }
...
export function allocateServerProductionIssue(
  ...
  const allocations: ProductionIssueLine['allocations'] = [];
  for (const row of sorted) {
    ...
    allocations.push(stripUndefined({
      ...
      shelf: row.shelf,
      quantity,
    }) as ProductionIssueLine['allocations'][number]);
    remaining -= quantity;
  }
...
async function loadLocationData(
  ...
  const balances: Array<LocationBalance & { itemType: InventoryItemType; itemId: string }> = [];
  ...
  for (const snap of defaultSnaps) {
    if (!snap.exists || clean(snap.data()?.tenantId) !== tenantId) continue;
    const data = snap.data() || {};
...
async function buildLines(
  ...
  const { balances, defaults } = await loadLocationData(
    actor.tenantId,
    warehouseId,
    [...aggregate.values()].map((row) => row.catalog),
  );
  return [...aggregate.entries()].map(([key, row]) => {
    const itemBalances = balances.filter((balance) => (
      `${balance.itemType}__${balance.itemId}` === key && Boolean(balance.locationId)
    ));
    const allocation = allocateServerProductionIssue(
      itemBalances,
      row.requiredQty,
      defaults.get(key),
    );
...
async function assertNoBlockingIssue(
  actor: ActorContext,
  ...
  if (blocking) {
    throw new HttpsError(
      'already-exists',
      'يوجد طلب/إذن صرف معلّق لنفس أمر الشغل أو الخطة. أنهِه أو ألغِه أولاً.',
      { orderId: blocking.id },
    );
  }
}

async function allocateReferenceAndCreate(actor: ActorContext, payload: IssueOrder) {
  ...
  return { id: created.id, ...created.data() } as IssueOrder;
}

export async function createProductionIssueDraftForActor(
  actor: ActorContext,
  ...
  if (!warehouseId) throw new HttpsError('invalid-argument', 'حدد مخزن صرف المكونات.');
  if (!(requestedQty > 0)) throw new HttpsError('invalid-argument', 'كمية الصرف يجب أن تكون أكبر من صفر.');
  ...
  if (!source.productId) throw new HttpsError('failed-precondition', 'المصدر غير مرتبط بمنتج صالح.');
  await assertNoBlockingIssue(actor, sourceType, source.id);
  const productSnap = await requireTenantDoc(
    COLLECTIONS.products,
    source.productId,
    actor.tenantId,
    'المنتج المرتبط غير موجود.',
  );
...
export async function prepareProductionIssueOrderForActor(
  actor: ActorContext,
  data: Record<string, unknown>,
): Promise<IssueOrder> {
  const orderId = clean(data.orderId);
  const orderSnap = await requireTenantDoc(
    COLLECTIONS.issueOrders,
    orderId,
    actor.tenantId,
    'طلب الصرف غير موجود.',
  );
  const order = { id: orderSnap.id, ...orderSnap.data() } as IssueOrder;
  ...
  if (!(quantity > 0)) throw new HttpsError('invalid-argument', 'كمية الاعتماد يجب أن تكون أكبر من صفر.');
  ...
  return { ...order, ...patch, id: orderSnap.id } as IssueOrder;
}

function shortageDetails(order: IssueOrder) {
  return order.lines
...
export async function approveAndIssueProductionIssueForActor(
  actor: ActorContext,
  data: Record<string, unknown>,
) {
  ...
  if (shortages.length) {
    throw new HttpsError(
      'failed-precondition',
      `لا يمكن اعتماد الصرف: يوجد عجز في ${shortages.length} مكوّن.`,
      { reason: 'insufficient-stock', shortages },
    );
  }
  ...
  return { order: { id: issuedSnap.id, ...issuedSnap.data() } as IssueOrder, result };
}

export const createProductionIssueDraft = onCall(
...
export const prepareProductionIssueOrder = onCall(
  { region: 'us-central1', memory: '512MiB' },
  async (request) => {
    const actor = await loadProductionIssueActor(requireAuthUid(request));
    assertCreatePermission(actor);
...
export const approveAndIssueProductionIssue = onCall(
  { region: 'us-central1', memory: '512MiB' },
  async (request) => {
    const actor = await loadProductionIssueActor(requireAuthUid(request));
