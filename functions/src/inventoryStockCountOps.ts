...
const roundQty = (value: unknown) => {
  ...
  if (!Number.isFinite(number) || number < 0) throw new HttpsError('invalid-argument', 'توجد كمية جرد غير صالحة.');
  return Math.round(number * 10000) / 10000;
};

export const createInventoryCountSessionHandler = async (request: CallableRequest) => {
  ...
  if (!requested.length || requested.length > 450) throw new HttpsError('invalid-argument', 'جلسة الجرد يجب أن تحتوي من 1 إلى 450 صنفًا في الملف الواحد.');
  ...
  const normalized = requested.map((line) => {
    ...
    if (keys.has(key)) throw new HttpsError('invalid-argument', `الصنف ${itemId} مكرر في ملف الجرد.`);
    keys.add(key);
  ...
  const balanceRefs = normalized.map((line) => db.collection(
    countScope === 'warehouse' ? 'stock_items' : 'stock_location_balances',
  ).doc(countScope !== 'warehouse'
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    const lines = balances.map((snap, index) => {
      ...
      const balance = snap.data() as Record<string, unknown> | undefined;
      if (
        !snap.exists
