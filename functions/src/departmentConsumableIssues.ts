/**
 * Secure callables for department consumable issues.
 * All stock deductions/returns and status transitions for this feature run here.
 */
import {
...
} from './departmentConsumableLocation.js';
...
const formatDciReference = (seq: number) =>
  `DCI-${String(Math.max(1, Math.floor(seq))).padStart(4, '0')}`;
...
const loadApprovalMode = async (tenantId: string): Promise<ApprovalMode> => {
  ...
  const data = snap.data() as {
    planSettings?: { departmentConsumableIssueApprovalMode?: string };
  };
...
const resolveDepartment = async (tenantId: string, departmentId: string) => {
  const id = String(departmentId || '').trim();
  if (!id) throw new HttpsError('invalid-argument', 'حدد القسم.');
  ...
  if (data.isActive === false) throw new HttpsError('failed-precondition', 'القسم غير نشط.');
...
const loadItemLocationBalances = async (tenantId: string, warehouseId: string, itemId: string) => {
  const snap = await db
...
const loadWarehouseItemQty = async (warehouseId: string, itemId: string): Promise<number> => {
...
const loadDefaultLocationId = async (warehouseId: string, itemId: string): Promise<string> => {
  const snap = await db.collection(DEFAULT_ITEM_LOCATIONS_COLLECTION).doc(
    `${warehouseId}__material__${itemId}`,
  ).get();
...
const rememberDefaultItemLocation = async (params: {
  ...
  const now = toIsoNow();
  await db.collection(DEFAULT_ITEM_LOCATIONS_COLLECTION).doc(
    `${params.warehouseId}__material__${params.itemId}`,
  ).set({
...
const resolveConsumableMaterial = async (tenantId: string, itemId: string) => {
  const id = String(itemId || '').trim();
  if (!id) throw new HttpsError('invalid-argument', 'حدد الصنف لكل بند.');
  ...
  for (const [itemId, quantity] of merged.entries()) {
    ...
    if (allocated.error) {
      throw new HttpsError(
        'failed-precondition',
        `الرصيد غير كافٍ للصنف ${material.itemName}. ${allocated.error}`,
      );
    }
...
async function postIssueMovements(params: {
  ...
  return db.runTransaction(async (t) => {
    ...
    if (!liveSnap.exists) throw new HttpsError('not-found', 'سند الصرف غير موجود.');
    const live = liveSnap.data() as IssueDoc;
...
export async function addDepartmentConsumableStockHandler(
  ...
  try {
    ...
    requirePermission(
      ...
      'لا تملك صلاحية إضافة مستهلكات للمخزن.',
    );
    ...
    assertOperationPathEnabledServer(
      settingsSnap.data(),
      STOCK_MOVE_OPERATION_KEY,
      CONSUMABLE_ADD_STOCK_PATH,
    );
    ...
    const balRef = db.collection(STOCK_ITEMS_COLLECTION).doc(
      balanceDocId(warehouse.id, line.itemId),
    );
    const locRef = line.locationId
      ? db.collection(STOCK_LOCATION_BALANCES_COLLECTION).doc(
        locationBalanceDocId(warehouse.id, line.locationId, line.itemId),
      )
      : null;
    const counterRef = db.collection(INVENTORY_COUNTERS_COLLECTION).doc(actor.tenantId);
    const now = toIsoNow();
    ...
    await writeAudit({
      ...
      action: 'add_stock',
      entityId: txRef.id,
      description: `إضافة مستهلك ${line.itemName} (${line.quantity} ${line.unit}) إلى ${warehouse.name}${
        line.locationCode ? ` — رف ${line.locationCode}` : ''
      }`,
      metadata: {
        warehouseId: warehouse.id,
        itemId: line.itemId,
...
export async function createDepartmentConsumableIssueHandler(
  request: CallableRequest,
): Promise<{ id: string; referenceNo: string; status: IssueStatus }> {
  try {
    const uid = requireAuth(request);
    const actor = await loadActor(uid);
    requirePermission(
      actor,
      ['departmentConsumables.create', 'inventory.transactions.create'],
      'لا تملك صلاحية إنشاء صرف مستهلكات.',
    );
    if (!actor.tenantId) throw new HttpsError('permission-denied', 'لا يمكن تحديد الشركة.');
...
export async function submitDepartmentConsumableIssueHandler(
  request: CallableRequest,
...
export async function approveDepartmentConsumableIssueHandler(
  request: CallableRequest,
): Promise<{ id: string; status: IssueStatus }> {
  try {
    const uid = requireAuth(request);
...
export async function rejectDepartmentConsumableIssueHandler(
  request: CallableRequest,
): Promise<{ id: string; status: IssueStatus }> {
  try {
    const uid = requireAuth(request);
...
export async function issueDepartmentConsumableIssueHandler(
  request: CallableRequest,
  ...
  try {
    ...
    requirePermission(
      ...
      'لا تملك صلاحية تنفيذ صرف المستهلكات.',
    );
    const issueId = String((request.data as { issueId?: string })?.issueId || '').trim();
    const { id } = await loadIssue(issueId, actor);
...
export async function cancelDepartmentConsumableIssueHandler(
  request: CallableRequest,
): Promise<{ id: string; status: IssueStatus }> {
  try {
    const uid = requireAuth(request);
...
export async function returnDepartmentConsumableIssueHandler(
  request: CallableRequest,
  ...
  try {
    ...
    requirePermission(
      actor,
      ['departmentConsumables.issue', 'inventory.transactions.create'],
      'لا تملك صلاحية تسجيل مرتجع مستهلكات.',
    );
      ...
      for (let i = 0; i < resolvedReturns.length; i += 1) {
        ...
        t.set(
          ...
          stripUndefined({
            ...
            quantity,
            unit: source.unit,
            note: `مرتجع مستهلكات ${current.referenceNo}${
...
export async function getDepartmentConsumableMonthlyReportHandler(
  request: CallableRequest,
): Promise<{
  month: string;
  departmentId?: string;
  ...
  try {
    ...
    requirePermission(
      actor,
      ['departmentConsumables.view', 'departmentConsumables.export', 'inventory.view'],
      'لا تملك صلاحية عرض تقرير مستهلكات الأقسام.',
    );
    const data = (request.data || {}) as {
      month?: string;
      departmentId?: string;
