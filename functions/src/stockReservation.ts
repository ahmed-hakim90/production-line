...
export type StockBalanceData = {
  tenantId?: string;
  quantity?: unknown;
  reservedQty?: unknown;
};

export const stockAvailableQty = (balance: StockBalanceData | null | undefined): number => {
...
export const stockReservedQty = (balance: StockBalanceData | null | undefined): number => {
  const reserved = Number(balance?.reservedQty || 0);
  return Number.isFinite(reserved) && reserved > 0 ? reserved : 0;
};

export const reservationShortageMessage = (
  qty: number,
  available: number,
  label?: string,
): string => {
...
const assertTenant = (balance: StockBalanceData | undefined, tenantId: string, label: string) => {
  if (!balance) return;
...
export const reserveStockInTx = (
  ...
  existing: StockBalanceData | undefined,
): void => {
  ...
  if (available + 1e-9 < qty) {
    throw new HttpsError(
      'failed-precondition',
      reservationShortageMessage(qty, available, input.label),
    );
  }
  if (!existing) {
    tx.set(ref, {
...
export const releaseStockInTx = (
  tx: Transaction,
  ref: DocumentReference,
  input: { tenantId: string; qty: number; label?: string },
  existing: StockBalanceData | undefined,
): void => {
  const qty = Number(input.qty);
 ...
 */
export const consumeReservedInTx = (
  tx: Transaction,
  ref: DocumentReference,
  input: { tenantId: string; qty: number; label?: string },
  existing: StockBalanceData | undefined,
