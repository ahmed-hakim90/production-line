...
type RequestDoc = {
  referenceNo: string;
  status: Status;
  /** Center warehouse (source of stock) */
  fromWarehouseId: string;
  fromWarehouseName: string;
  /** Central warehouse (destination) */
...
const hasPerm = (actor: ActorContext, key: string): boolean =>
  actor.isSuperAdmin || actor.permissions[key] === true;

const requirePerm = (actor: ActorContext, key: string, message: string) => {
  if (!hasPerm(actor, key)) throw new HttpsError('permission-denied', message);
};
...
const loadWarehouse = async (tenantId: string, warehouseId: string) => {
...
const assertBoundIsCentral = async (actor: ActorContext): Promise<{ id: string; name: string }> => {
  if (!actor.boundWarehouseId && !actor.isSuperAdmin) {
    throw new HttpsError(
      'failed-precondition',
      'يجب ربط الحساب بمخزن قطع الغيار المركزي لعرض أرصدة المراكز أو إنشاء سحب.',
    );
  }
  if (actor.isSuperAdmin && !actor.boundWarehouseId) {
    ...
    if (!active) throw new HttpsError('failed-precondition', 'لا يوجد مخزن قطع غيار مركزي.');
...
export const listMaintenanceCenterSpareBalancesHandler = async (request: CallableRequest) => {
  const uid = requireAuth(request);
  const actor = await loadActor(uid);
  if (
    !hasPerm(actor, 'sparePartsRecall.view')
  ...
  const data = (request.data || {}) as { warehouseId?: string; search?: string };
  ...
  for (const center of centers) {
    ...
    for (const doc of balSnap.docs) {
      const bal = doc.data() as Record<string, unknown>;
  ...
  return { ok: true as const, rows, centers };
};

export const createSparePartsRecallHandler = async (request: CallableRequest) => {
  const uid = requireAuth(request);
  const actor = await loadActor(uid);
  if (
  ...
  if (!fromWarehouseId) throw new HttpsError('invalid-argument', 'حدد مخزن المركز.');
  ...
  if (!rawLines.length) throw new HttpsError('invalid-argument', 'أضف بنداً واحداً على الأقل.');
  ...
  for (const row of rawLines) {
    ...
    if (!(qty > 0)) throw new HttpsError('invalid-argument', 'كمية كل بند يجب أن تكون أكبر من صفر.');
    if (seen.has(itemId)) throw new HttpsError('invalid-argument', 'لا تكرر نفس المكوّن.');
    seen.add(itemId);

    const matSnap = await db.collection(MATERIALS).doc(itemId).get();
    if (!matSnap.exists) throw new HttpsError('not-found', `المكوّن ${itemId} غير موجود.`);
    const mat = matSnap.data() as {
      tenantId?: string;
      name?: string;
      code?: string;
      baseUnit?: string;
    ...
    if (qty > available + 1e-9) {
      throw new HttpsError(
        'failed-precondition',
        `الكمية المطلوبة لـ ${mat.name || itemId} أكبر من رصيد المركز (${available}).`,
      );
    }
...
export const confirmSparePartsRecallHandler = async (request: CallableRequest) => {
  const uid = requireAuth(request);
  const actor = await loadActor(uid);
  if (
    !hasPerm(actor, 'sparePartsRecall.confirm')
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!fresh.exists) throw new HttpsError('not-found', 'طلب السحب غير موجود.');
    const data = fresh.data() as RequestDoc;
    ...
    for (const line of data.lines || []) {
      ...
      if (qty > sourceQty + 1e-9) {
        throw new HttpsError(
          'failed-precondition',
          `رصيد المركز غير كافٍ للصنف ${line.itemName || line.itemId}.`,
        );
      }
...
export const cancelSparePartsRecallHandler = async (request: CallableRequest) => {
  const uid = requireAuth(request);
  const actor = await loadActor(uid);
  if (
    !hasPerm(actor, 'sparePartsRecall.cancel')
    && !hasPerm(actor, 'sparePartsRecall.create')
