...
const round = (value: unknown) => {
  ...
  if (!Number.isFinite(n) || n <= 0) throw new HttpsError('invalid-argument', 'كل كمية إضافة يجب أن تكون أكبر من صفر.');
  return Math.round(n * 10000) / 10000;
};

const catalogCollection = (itemType: string) => itemType === 'finished_good'
  ? 'products'
  : itemType === 'raw_material' ? 'raw_materials' : 'materials';

export const postGeneralStockReceiptHandler = async (request: CallableRequest) => {
  ...
  if (!warehouseSnap.exists || clean(warehouseSnap.data()?.tenantId) !== tenantId) throw new HttpsError('not-found', 'المخزن غير موجود داخل الشركة.');
  ...
  if (workOrderId) {
    ...
    if (!snap.exists || clean(snap.data()?.tenantId) !== tenantId) throw new HttpsError('not-found', 'أمر الشغل غير موجود داخل الشركة.');
  }
  ...
  if (issueRef && (!issueSnap?.exists || clean(issueSnap.data()?.tenantId) !== tenantId)) throw new HttpsError('not-found', 'إذن الصرف الأصلي غير موجود داخل الشركة.');

  const requested = Array.isArray(data.lines) ? data.lines as Array<Record<string, unknown>> : [];
  if (!requested.length || requested.length > 100) throw new HttpsError('invalid-argument', 'إذن الإضافة يجب أن يحتوي من 1 إلى 100 بند.');
  ...
  const lines = requested.map((line) => {
    ...
    if (seen.has(key)) throw new HttpsError('invalid-argument', 'لا يمكن تكرار نفس البند واللوكيشن.');
    seen.add(key);
  ...
  const catalogRows = catalogSnaps.map((snap, index) => {
    ...
    if (!snap.exists || clean(row?.tenantId) !== tenantId) throw new HttpsError('not-found', `الصنف ${lines[index].itemId} غير موجود داخل الشركة.`);
    return row!;
  });
  ...
  locationSnaps.forEach((snap, index) => {
    if (!snap) return;
    const row = snap.data();
    if (!snap.exists || clean(row?.tenantId) !== tenantId || clean(row?.warehouseId) !== warehouseId) throw new HttpsError('not-found', `اللوكيشن في البند ${index + 1} لا يتبع المخزن.`);
  });
  ...
  await db.runTransaction(async (tx) => {
    ...
    const postedLines = lines.map((line, index) => {
      ...
      const balance = balances[index].data() as Record<string, unknown> | undefined;
      const location = locationBalances[index]?.data() as Record<string, unknown> | undefined;
      ...
      if (originalIssue) {
        ...
        if (original <= 0 || Number(returned[key] || 0) + line.quantity > original + 0.0001) throw new HttpsError('failed-precondition', `مرتجع ${itemName} أكبر من الكمية المصروفة.`);
