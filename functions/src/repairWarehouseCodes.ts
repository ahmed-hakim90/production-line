...
export function formatRepairWarehouseCode(
...
export function parseRepairWarehouseSequence(code: string): number | null {
...
export function maxRepairWarehouseSequence(codes: Array<string | null | undefined>): number {
  let max = 0;
...
export async function allocateRepairWarehouseSequence(tenantId: string): Promise<number> {
  ...
  const warehouses = await db
    ...
    .limit(2000)
    .get();
  const maxFromCodes = maxRepairWarehouseSequence(
    warehouses.docs.map((snap) => String(snap.data()?.code || '')),
  );
  let next = 1;
  await db.runTransaction(async (tx) => {
    ...
    tx.set(
      counterRef,
      { tenantId, value: next, updatedAt: nowIso() },
      { merge: true },
    );
  });
  return next;
