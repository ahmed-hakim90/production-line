...
const inPeriod = (value: unknown, from: string, to: string): boolean => {
  const day = String(value || '').slice(0, 10);
  if (!day) return !from && !to;
  return (!from || day >= from) && (!to || day <= to);
};

const sumPartsActualCost = (value: unknown): number => money(
  (Array.isArray(value) ? value : []).reduce((sum, raw) => {
...
const loadActor = async (request: CallableRequest) => {
  const uid = String(request.auth?.uid || '').trim();
  if (!uid) throw new HttpsError('unauthenticated', 'يجب تسجيل الدخول.');
  const userSnap = await db.collection('users').doc(uid).get();
  const user = userSnap.data() as Row | undefined;
  ...
  if (!allowed) throw new HttpsError('permission-denied', 'ليس لديك صلاحية عرض العملاء.');
  return { uid, tenantId };
};

const getDocsByIds = async (collection: string, ids: string[]) => {
  if (ids.length === 0) return [] as FirebaseFirestore.DocumentSnapshot[];
  return db.getAll(...ids.map((id) => db.collection(collection).doc(id)));
};

const getPaymentsForJobs = async (jobIds: string[], tenantId: string) => {
...
export async function getCustomerFinancialAnalyticsHandler(request: CallableRequest) {
  ...
  if (!customerId) throw new HttpsError('invalid-argument', 'العميل مطلوب.');
  if (from && to && from > to) throw new HttpsError('invalid-argument', 'فترة التحليل غير صالحة.');
  ...
  const financialByJob = new Map(financialSnaps.filter((snap) => snap.exists).map((snap) => [snap.id, snap.data() as Row]));

  const repairRows = jobDocs.map((doc) => {
    const job = doc.data() as Row;
  ...
  const invoiceRows = invoicesSnap.docs
    ...
    .map((row) => ({
      ...
      netAmount: money(row.total), quantity: money((Array.isArray(row.lines) ? row.lines : []).reduce((sum, raw) => sum + money((raw as Row).quantity), 0)),
  ...
  return { ok: true as const, customerId, period: { from, to }, summary, repairRows, invoiceRows, paymentRows };
}
