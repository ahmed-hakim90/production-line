...
const repairMaintenanceWarehouseName = (branchName: string): string =>
  `مخزن صيانة - ${String(branchName || '').trim() || 'فرع'}`;

const repairCenterWarehouseId = (branchId: string): string =>
  `repair-center-${String(branchId || '').trim()}`;

const repairCostCenterCode = (sequence: number): string => {
...
function requireCreatePermission(actor: Actor): void {
  if (actor.isSuperAdmin || actor.permissions[CREATE_PERMISSION] === true) return;
  throw new HttpsError('permission-denied', 'لا تملك صلاحية إدارة المراكز.');
}

async function allocateMaintenanceWarehouseCode(tenantId: string): Promise<{
  sequence: number;
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    const taken = await db
      ...
      .limit(1)
      .get();
    if (taken.empty) return { sequence, warehouseCode };
  }
  throw new HttpsError('already-exists', 'تعذر توليد كود مخزن فريد.');
}

async function nextCostCenterCode(tenantId: string): Promise<string> {
  ...
  await db.runTransaction(async (tx) => {
    ...
    tx.set(
      counterRef,
      { tenantId, value: sequence, updatedAt: nowIso() },
      { merge: true },
    );
  });
    ...
    if (!usedCodes.has(code)) {
      if (offset > 0) {
        await counterRef.set(
          { tenantId, value: sequence + offset, updatedAt: nowIso() },
          { merge: true },
        );
      }
      return code;
...
async function clearOtherMainBranches(tenantId: string, exceptId: string): Promise<void> {
...
export async function createRepairBranchProvisionedHandler(request: CallableRequest) {
  const actor = await actorFromRequest(request);
  requireCreatePermission(actor);

  const data = (request.data || {}) as Json;
  const name = text(data.name, 160);
  if (!name) throw new HttpsError('invalid-argument', 'اسم الفرع مطلوب.');
  ...
  const employee = employeeSnap.data() as Json;
  ...
  const managerEmployeeName =
    text(data.managerEmployeeName, 160) || text(employee.name, 160);
  ...
  try {
    ...
    throw new HttpsError(
      'failed-precondition',
      `تم إنشاء الفرع لكن ${message} احذف الفرع ثم أعد المحاولة.`,
    );
  }
  ...
  try {
    ...
    throw new HttpsError(
      'failed-precondition',
      `تم إنشاء الفرع والمخازن لكن ${message} اربطه من الحسابات أو احذف الفرع ثم أعد المحاولة.`,
    );
  }
  ...
  try {
    ...
    throw new HttpsError(
      'failed-precondition',
      `تم إنشاء الفرع والمخازن ومركز التكلفة لكن ${message} أكمل الربط من الحسابات ← إعدادات الحسابات.`,
    );
  }
