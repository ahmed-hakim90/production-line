...
const strictPriceValues = (value: unknown, label: string): PriceValues => {
  ...
  const parse = (key: keyof PriceValues): number => {
...
export const normalizeRepairPartsPricingUpdates = (value: unknown): PricingUpdate[] => {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_UPDATES_PER_REQUEST) {
    throw new HttpsError(
      'invalid-argument',
      `يجب إرسال من 1 إلى ${MAX_UPDATES_PER_REQUEST} تحديث في الطلب الواحد.`,
    );
  }
...
const pricesFromMaterial = (material: Record<string, unknown>): PriceValues => ({
...
const samePrices = (left: PriceValues, right: PriceValues): boolean => (
  left.consumer === right.consumer
  && left.trader === right.trader
  && left.cost === right.cost
);

export const canManageRepairPartsPricing = (
  actor: Pick<Actor, 'isSuperAdmin' | 'permissions'>,
): boolean => actor.isSuperAdmin || actor.permissions['repair.pricing.manage'] === true;

export const validateRepairPartsPricingMaterial = (
  ...
  if (!samePrices(storedPrices, update.current)) {
    throw new HttpsError(
      'failed-precondition',
      `تم تعديل أسعار ${update.code} بعد تحميل الملف. حدّث الصفحة وأعد المحاولة.`,
    );
  }
  return storedPrices;
};

export const updateRepairPartsPricingHandler = async (request: CallableRequest) => {
  ...
  const updatedCount = await db.runTransaction(async (tx) => {
    ...
    const validated = snapshots.map((snapshot, index) => {
      ...
      if (!snapshot.exists) throw new HttpsError('not-found', `المادة ${update.code} غير موجودة.`);
      const material = snapshot.data() as Record<string, unknown>;
    ...
    validated.forEach(({ update, storedPrices }, index) => {
      if (samePrices(storedPrices, update.next)) return;
      changed += 1;
