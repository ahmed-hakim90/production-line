...
type PaymentCollectionKind = 'deposit' | 'receivable';
...
async function assertBranchAllowsCreditDelivery(actor: Actor, branchId: string) {
 ...
 */
async function collectReceivable(actor: Actor, data: Record<string, unknown>) {
  requirePermission(actor, ['repair.payments.collect'], 'ليس لديك صلاحية تحصيل دفعة.');
  const authorizationId = String(data.authorizationId || '').trim();
  const paymentId = String(data.requestId || '').trim().replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 100);
  const method = String(data.method || '') as PaymentMethod;
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    if (isWarrantySettlement(fin) || isWarrantySettlement(authSnap.data() as Record<string, unknown>)) {
    ...
    if (balance <= 0) throw new HttpsError('failed-precondition', 'لا يوجد رصيد ذمم متبقٍ.');
    if (amount > balance + 0.001) throw new HttpsError('invalid-argument', 'مبلغ التحصيل أكبر من الذمة المتبقية.');
...
async function reversePayment(actor: Actor, data: Record<string, unknown>) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const collectionKind = String(paymentSnap.data()?.collectionKind || 'deposit') as PaymentCollectionKind;
    const isReceivableCollect = collectionKind === 'receivable';
    const jobClosed = Boolean(
      jobSnap.exists
...
export const mutateRepairPaymentHandler = async (request: CallableRequest) => {
  ...
  if (operation === 'collect_receivable') return collectReceivable(actor, data);
