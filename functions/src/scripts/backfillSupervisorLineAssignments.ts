...
const run = async () => {
  ...
  const linesWithActive = new Set(
    existingSnap.docs
      .map((d) => String((d.data() as { lineId?: string }).lineId || '').trim())
      .filter(Boolean),
  );
  ...
  let lastReportDoc: QueryDocumentSnapshot | null = null;
  ...
  for (;;) {
    ...
    for (const row of snap.docs) {
      const data = row.data() as { lineId?: string; employeeId?: string; date?: string };
      const lineId = String(data.lineId || '').trim();
      const supervisorId = String(data.employeeId || '').trim();
      const date = String(data.date || '').trim();
      if (!lineId || !supervisorId || !isYmd(date)) continue;
  ...
  let lastWorkOrderDoc: QueryDocumentSnapshot | null = null;
  for (;;) {
    let q = db.collection(WORK_ORDERS_COLLECTION).orderBy('__name__').limit(args.pageSize);
    if (lastWorkOrderDoc) q = q.startAfter(lastWorkOrderDoc);
    const snap = await q.get();
    if (snap.empty) break;
    workOrderReads += snap.size;
    for (const row of snap.docs) {
      const data = row.data() as { lineId?: string; supervisorId?: string; targetDate?: string };
      const lineId = String(data.lineId || '').trim();
  ...
  activeAfter.docs.forEach((docSnap) => {
    const data = docSnap.data() as { lineId?: string; effectiveFrom?: string; effectiveTo?: string };
    const lineId = String(data.lineId || '').trim();
    const from = String(data.effectiveFrom || '').trim();
    const to = String(data.effectiveTo || '').trim();
    if (!lineId || !from || from > validateDate) return;
    if (to && to < from) return;
    if (to && to < validateDate) return;
