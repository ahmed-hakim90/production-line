...
function readDefaultProjectFromFirebaserc(): string | undefined {
  ...
  for (const p of candidates) {
    try {
      ...
      const j = JSON.parse(raw) as { projects?: { default?: string } };
...
function resolveProjectId(argv: string[]): string {
  const get = (f: string) => {
...
const run = async () => {
  ...
  if (!args.slug || !args.name) {
    console.error(
      'Required: --slug <slug> --name <name> [--project <id>] [--credentials <serviceAccount.json>] [--phone] [--address] [--apply]',
    );
    process.exit(1);
  }
  ...
  if (!args.projectId) {
    console.error(
      'Missing Firebase/GCP project id. Use: --project <id> or set GCLOUD_PROJECT, or add projects.default in .firebaserc next to firebase.json.',
    );
    process.exit(1);
  }
  ...
  for (const name of COLLECTIONS_TO_MIGRATE) {
    ...
    if (updated > 0) summary[name] = updated;
  }
