...
export const PrintReportLayout = forwardRef<HTMLDivElement, PrintReportLayoutProps>(
  ({ companyName, reportType, printDate, meta, kpis, sections, signatures, version = "v4.0.57" }, ref) => {
    return (
      <div
        id="print-root"
        ref={ref}
        dir="rtl"
        className="print-root print-report bg-white w-[640px] mx-auto p-9 print:p-0 print:w-full"
        style={{ fontFamily: "'Cairo', 'Noto Sans Arabic', Tahoma, sans-serif", fontSize: "13px" }}
      >
        <div className="flex items-start justify-between pb-3 mb-4 border-b-2 border-indigo-500">
          <div>
          ...
          <div className="flex flex-col items-end gap-1">
            <span className="bg-indigo-50 text-indigo-700 text-[13px] font-semibold px-3 py-1 rounded-md">
              {reportType}
            </span>
        ...
        <div className="grid grid-cols-4 mb-4 border border-slate-200 rounded-lg overflow-hidden">
          {[
            ...
            <div key={i} className={cn("px-3 py-2 bg-slate-50", i < 3 && "border-l border-slate-200")}>
              <p className="text-[9px] text-slate-400 tracking-wide mb-1">{item.label}</p>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-indigo-50 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${Math.max(0, Math.min(100, section.progress.value))}%` }}
                          />
                        </div>
                        <span className="text-xs font-bold text-indigo-600 tabular-nums min-w-[32px] text-left">
        ...
        <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-200">
          <p className="text-[10px] text-slate-400">هذا التقرير تم إنشاؤه آلياً من نظام إدارة الإنتاج — {printDate}</p>
          <p className="text-[10px] font-bold text-indigo-500">HAKIM {version}</p>
        </div>
      </div>
    )
  },
