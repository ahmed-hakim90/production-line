          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {!isInstalled && deferredPrompt && (
              <button
                ...
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-colors"
              >
            ...
            {/* Read-only */}
            {isReadOnly && (
              <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
              ...
              <div key={group.key} className="relative h-full flex items-center">
                <button
                  ...
                  className={[
                    ...
                    active
                      ? 'text-primary'
                      : 'text-slate-600 dark:text-[var(--color-text-muted)] hover:text-slate-900 dark:hover:text-white',
                  ].join(' ')}
                >
        ...
        >
                          ...
                          <NavLink
                            ...
                            className={[
                              ...
                              itemActive
                                ...
                                : 'text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-white dark:hover:bg-slate-800 font-medium',
                            ].join(' ')}
                          >
          ...
          <div className="shrink-0 border-t border-[var(--color-border)] px-4 py-3 space-y-2 bg-slate-50/60 dark:bg-slate-800/20">
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-white dark:hover:bg-slate-800 transition-colors text-start"
            >
            ...
            {can('selfService.view') && (
              <NavLink
                to="/self-service"
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-white dark:hover:bg-slate-800 transition-colors"
              >
    ...
    {profileOpen && (
      <div className="absolute left-0 top-full mt-2 w-60 bg-[var(--color-card)] rounded-2xl border border-[var(--color-border)] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 z-50">
        ...
        <div className="p-1.5">
          <button
            onClick={onOpenTasks}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-start"
          >
            <span className="material-icons-round text-lg text-slate-400">task_alt</span>
            <span className="flex-1">المهام</span>
            {activeTasksCount > 0 && (
          ...
          {can('selfService.view') && (
            <NavLink
              to="/self-service"
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            >
              <span className="material-icons-round text-lg text-slate-400">account_circle</span>
              <span>ملفي الشخصي</span>
            </NavLink>
...
const DropdownItem: React.FC<DropdownItemProps> = ({ item, isActive, badge, iconBgClass }) => (
  <NavLink
    ...
    className={[
      ...
      isActive
        ? 'bg-primary/10 text-primary font-bold'
        : 'text-slate-600 dark:text-[var(--color-text-muted)] hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white font-medium',
    ].join(' ')}
  >
    <div className={[
      ...
      isActive
        ...
        : 'bg-slate-100 dark:bg-slate-800 text-slate-500 group-hover:bg-[#e8eaed] dark:group-hover:bg-slate-700',
    ].join(' ')}>
