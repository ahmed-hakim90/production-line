...
تُحسب **يوميًا للمشرف على نفس الخط** ثم تُوزع على تقاريره في هذا اليوم:

- تكلفة يوم المشرف =  
  - `supervisorIndirectCost` (إن كانت محفوظة وموجبة)  
  - أو `supervisorHourlyRate × max(workHours)` لتقارير نفس `(line + date + supervisor)`
- ثم يتم توزيعها على التقارير بنسبة الكمية المنتجة لكل تقرير داخل نفس المجموعة.

> بهذا لا تتكرر "يومية المشرف" بعدد التقارير (لا يوجد ضرب 3 يوميات عند وجود 3 تقارير).
