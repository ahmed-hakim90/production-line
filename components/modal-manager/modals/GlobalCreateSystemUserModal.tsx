...
export const GlobalCreateSystemUserModal: React.FC = () => {
  ...
  const handleSubmit = async () => {
    if (!displayName.trim() || !email.trim() || !password.trim() || !roleId) {
      setMessage({ type: 'error', text: 'أدخل الاسم والبريد وكلمة المرور والدور.' });
      return;
    }
    setSubmitting(true);
    setMessage(null);
    try {
      ...
      setMessage({ type: 'error', text: error?.message || 'تعذر إنشاء المستخدم.' });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="bg-[var(--color-card)] rounded-[var(--border-radius-xl)] shadow-2xl w-full max-w-2xl border border-[var(--color-border)]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input className="w-full border border-[var(--color-border)] rounded-[var(--border-radius-base)] px-3 py-2 text-sm bg-[var(--color-card)] text-[var(--color-text)]" placeholder="الاسم" value={displayName} onChange={(e) => setDisplayName(e.target.value)} />
            <input className="w-full border border-[var(--color-border)] rounded-[var(--border-radius-base)] px-3 py-2 text-sm bg-[var(--color-card)] text-[var(--color-text)]" placeholder="البريد الإلكتروني" value={email} onChange={(e) => setEmail(e.target.value)} />
            <input className="w-full border border-[var(--color-border)] rounded-[var(--border-radius-base)] px-3 py-2 text-sm bg-[var(--color-card)] text-[var(--color-text)]" placeholder="كلمة المرور" value={password} onChange={(e) => setPassword(e.target.value)} />
          ...
          <div>
            <label className="text-xs font-bold text-[var(--color-text-muted)] block mb-1">ربط بموظف (اختياري)</label>
            <SearchableSelect
